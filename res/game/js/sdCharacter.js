...
class sdCharacter{
	static init_class(){
		...
		*/
		...
		sdCharacter.collision_dots = [];sdCharacter.collision_normals = [];sdCharacter.collision_dots_sit = [];
		for ( var r = 0; r < 2; r++ )
		for ( var an = 0; an < 8; an++ ){
			...
			for ( var v = 0; v <= 8; v++ ){
				...
				if ( v === 0 ) sdCharacter.collision_normals.push( new THREE.Vector3( 0, 1, 0 ) );
				else if ( v === 8 ) sdCharacter.collision_normals.push( new THREE.Vector3( 0, -1, 0 ) );
				else sdCharacter.collision_normals.push( new THREE.Vector3( -Math.sin( ang ), 0, -Math.cos( ang ) ) );
	...
	ApplyLimbImmortalityIfNeeded(){ if ( !sdNet.MP_mode ) if ( main.my_character === this ){for ( var g = 0; g < this.atoms.length; g++ ){this.atoms[ g ].hp = 99999999;}}}
	
	Ressurect( was_me ){
		var old_char = this;
		this.shadow.visible = true;
		
		if ( sdCharacter.characters.indexOf( old_char ) === -1 ) sdCharacter.characters.push( old_char );
		else console.log('Ressurect on non-removed character? Not adding it "back" to list of characters.');
		...
		if ( was_me ) main.SetActiveCharacter( old_char );
	...
	RestoreLimbs( spawn_pseudo_atoms=true ) // spawn_pseudo_atoms also means that color needs to be reset, not just alpha
	{
		...
		for ( var i = 0; i < old_char.atoms[ g ].length; i++ ) {
			...
			if ( !old_char.atoms[ g ][ i ].removed ){sdAtom.pseudo_atoms.push( new PseudoAtom( old_char.atoms[ g ][ i ] ) );}
			
			old_char.atoms[ g ][ i ].removed = false;old_char.atoms[ g ][ i ].material = old_char.atoms[ g ][ i ].material_initial;
			
			if ( spawn_pseudo_atoms ){
				old_char.atoms[ g ][ i ].r = old_char.atoms[ g ][ i ].r_initial;old_char.atoms[ g ][ i ].g = old_char.atoms[ g ][ i ].g_initial;old_char.atoms[ g ][ i ].b = old_char.atoms[ g ][ i ].b_initial;}
	...
	Respawn( pos=true ) // pos also means it is "me"
	{
		if ( pos || !main.MP_mode )
		{
			...
			if ( sdAI.use_brain_js ){sdAI.RunTraining();}
		...
		this.last_valid_x = this.x;this.last_valid_y = this.y;this.last_valid_z = this.z;
	...
	DealDamage( d, from=null, x=0, y=0, z=0 ){
		if ( this.hea > 0 ){
			if ( from !== null && from !== this ){this.last_attacker = from;this.last_attacker_time = pb2_mp.ticks_passed;}
			else{
				if ( this.last_attacker !== null )
				if ( pb2_mp.ticks_passed - this.last_attacker_time < 7000 ){from = this.last_attacker;}}
			
			
			if ( this.hea > 30 && this.hea - d <= 30 ) if ( this.ai !== null ) this.ai.ResetFavGun();
		
			this.hea -= d; this.regen_timer = 0;
				...
				if ( this.team !== main.mp_character.team ){
					...
					if ( main.report_damage ){
						...
						if ( this.hea <= 0 ){
							...
							setTimeout( function(){sdSound.PlayInterfaceSound({ sound: lib.frag_report, volume: 6 });}, 1000 );}}}
				else{
					if ( main.report_damage ){
						main.HitPulse( -d );
			...
			if ( this.team === main.mp_character.team ){color = '255,100,100,1';}
			
			//if ( this.hurt_anim === 0 )
			//{
				this.hurt_direction = ( Math.random() < 0.5 ? 1 : -1 );this.hurt_direction2 = ( Math.random() < 0.5 ? 1 : -1 );
			//}
			this.hurt_anim = 5;
	...
	SetBodyPartMaterial( all, m, initial=false ){
		...
		for ( var i = 0; i < all.length; i++ ){
			...
			else{a.tox += r.x;a.toy += r.y;a.toz += r.z;
			}}}
	HookHere( x, y, z, di=-1 )
	{
		this.hook_pos.x = x;
	...
	remove( respawn=true ){
		...
		let was_me = false;
		if ( main.my_character === this ){
			main.main_camera.position.x += this.look_direction.x * 10;main.main_camera.position.y += this.look_direction.y * 10;main.main_camera.position.z += this.look_direction.z * 10;
		...
		if ( i >= 0 ){
			...
			if ( respawn ){
				...
				setTimeout( function(){
						if ( !main.game_loop_started )return;

						for ( var i = 0; i < ragdoll_chains.length; i++ ) ragdoll_chains[ i ].remove();
						/*
						if ( was_me )
						if ( !main.MP_mode )
						main.GAME_FPS = 30;
						*/
						if ( was_me || !main.MP_mode ) {
							...
							if ( main.MP_mode ) sdSync.MP_SendEvent( sdSync.COMMAND_I_RESSURECT );}
	...
	*/
	get reload_timer(){debugger;
		//return this.cur_weapon_object.reload_timer;
	}
	set reload_timer( v ){debugger;
		//this.cur_weapon_object.reload_timer = v;
	}
	
	get cur_weapon_mesh(){return this.inventory[ this.cur_weapon_slot ].mesh;}
	get cur_weapon_object(){return this.inventory[ this.cur_weapon_slot ];}
	set cur_weapon_mesh( v ){debugger; // Set this.cur_weapon_slot instead?
		//return this.inventory[ this.cur_weapon_slot ].mesh;
	}
	
	get reload_timers(){debugger;}
	...
	constructor( params ){
		...
		this.x = params.x;this.y = params.y;this.z = params.z;
		...
		this.muzzle_r = 0;this.muzzle_g = 0;this.muzzle_b = 0;this.muzzle_a = 0;
		...
		this.last_valid_x = this.x;this.last_valid_y = this.y;this.last_valid_z = this.z;this.last_valid_sit = 0;
		/*
		this._tox = 0;
		this._toy = 0;
		this._toz = 0;
		*/
		...
		this.stand = false;this.stand_timer = 0; // Whenever above 0 - player can play walk animation
		...
		this.arm2 = new THREE.Object3D();this.arm2.position.x = -1;this.arm2.position.y = 7;this.arm2.position.z = -3;this.arm2.rotation.y = 0.3;
		this.body.add( this.arm2 );
		
		this.leg1a = new THREE.Object3D();this.leg1a.position.z = 2;this.leg1a.position.y = -2;
		this.mesh.add( this.leg1a );
		//
			this.leg1b = new THREE.Object3D();this.leg1b.position.x = -6;this.leg1b.position.y = 0;
			this.leg1a.add( this.leg1b );
		
		this.leg2a = new THREE.Object3D();this.leg2a.position.z = -2;this.leg2a.position.y = -2;
		this.mesh.add( this.leg2a );
		//
			this.leg2b = new THREE.Object3D();this.leg2b.position.x = -6;this.leg2b.position.y = 0;
		...
		f.position.x = -8;f.position.y = 3;
		this.rifle.add( f );
		
		var f = new THREE.Object3D();
		f.position.x = -7;f.position.y = 1;
		this.rocket.add( f );
		
		var f = new THREE.Object3D();
		f.position.x = -8;f.position.y = 2;
		this.sniper.add( f );
		
		var f = new THREE.Object3D();f.position.x = -8;f.position.y = 2;
		this.shotgun.add( f );
		...
		var offset_x = 0;var offset_y = 0;var offset_z = 0;
		...
		for ( var y = 0; y < 16; y++ ){
			...
			var r = rgba.r / 255;var g = rgba.g / 255;var b = rgba.b / 255;
			...
			for ( var z = 0; z < tolerance_ceil; z++ ){
				sdAtom.atoms.push( new sdAtom( x * sdAtom.atom_scale + offset_x, 
					...
					-x,-y,z / tolerance_ceil * tolerance,
					this,glowing ) );
		...
		function ConnectBodyPart( x1, y1, w, h ){
			x1 *= sdAtom.atom_scale;y1 *= sdAtom.atom_scale;
			w *= sdAtom.atom_scale;h *= sdAtom.atom_scale;
		...
		function SetOrigin( all, x, y, z ){
			for ( var i = 0; i < all.length; i++ ){
				...
				a.model_x = a.model_x + x;a.model_y = a.model_y + y;a.model_z = a.model_z - z;}}
		...
		SetOrigin( arm1, 8, 0, 0 );SetOrigin( arm2, 8, 0, 0 );
		SetOrigin( leg1a, 20, 2, 0 );SetOrigin( leg2a, 20, 2, 0 );SetOrigin( leg1b, 26, 3, 0 );SetOrigin( leg2b, 26, 3, 0 );
			...
			for ( var i = 0; i < all.length; i++ ){
				...
				if ( i !== i2 ){
					...
					if ( di < 9 )
					{
						...
						a.r_initial = a.r = a.r * br;a.g_initial = a.g = a.g * br;a.b_initial = a.b = a.b * br;}}
				// origin
				if ( all !== body ){
				...
				a.r_initial = a.r = a.r * 1.25;a.g_initial = a.g = a.g * 1.25;a.b_initial = a.b = a.b * 1.25;}}

		Occlusion( body );
		Occlusion( arm1 );Occlusion( arm2 );
		Occlusion( leg1a );Occlusion( leg2a );Occlusion( leg1b );Occlusion( leg2b );
		Occlusion( head );
		
		sdCharacter.atoms_per_player = sdAtom.atoms.length - context_atoms_from;
		
		this.trace_line_direction_normal = new THREE.Vector3();this.trace_line_percentage = 0;
	...
	TraceLineAllDirection( cx, cy, cz, dx, dy, dz, step_size, sit ){
		...
		for ( var i2 = 0; i2 < sdCharacter.collision_dots.length; i2++ ){
			...
			var dotx = d.x;var doty = d.y * ( 1 - sit ) + sdCharacter.collision_dots_sit[ i2 ].y * sit;var dotz = d.z;

			var fx = cx + dotx;var fy = cy + doty;var fz = cz + dotz;

			var tx = fx + dx;var ty = fy + dy;var tz = fz + dz;
	...
	static ApplyTeamColorToObject( a, team ){
		...
		function Set( r,g,b ){
			...
			a.r = br * r * 0.9 + br * 0.1;a.g = br * g * 0.9 + br * 0.1;a.b = br * b * 0.9 + br * 0.1;
			/*
			a.r = 1;
			a.g = 1;
			a.b = 1;*/
		...
		if ( team === 0 )
		{
			if ( Delta( 0, 0, 128 ) ){Set( 0, 128 / 255 * 0.5, 128 / 255 );}
			else if ( Delta( 255, 0, 0 ) ){Set( 0, 1 * 0.5, 1 );}
			else if ( Delta( 128, 128, 0 ) ) { Set( 0, 0, 0 ); } }
		else if ( team === 1 ) {
			if ( Delta( 0, 0, 128 ) ) {Set( 128 / 255, 128 / 255 * 0.666, 0 );}
			else if ( Delta( 255, 0, 0 ) ) { Set( 1, 1 * 0.666, 0 ); }
			else if ( Delta( 128, 128, 0 ) ) { Set( 0, 0, 0 ); } }
		else if ( team === 2 ) {
			/*if ( a.r * 255 === 0 &&
			     a.g * 255 === 0 &&
	...
	UpdateCharacter( GSPEED, teleport_limb_mode )
	{
		...
		if ( c.hurt_timeout > 0 )c.hurt_timeout -= GSPEED;
		...
		if ( c.x < 0 || c.z < 0 || c.x > main.level_chunks_x * main.chunk_size || c.z > main.level_chunks_z * main.chunk_size ){
			...
			if ( c.y < di_out ){
				var dx = in_game_x - c.x;var dz = in_game_z - c.z;

				var di = main.Dist3D( dx, dz, 0, 0, 0, 0 );
				if ( di > 0.01 ){
					dx /= di;dz /= di;

					c.tox = main.MorphWithTimeScale( c.tox, 0, 0.9, GSPEED );c.toy = main.MorphWithTimeScale( c.toy, 0, 0.9, GSPEED );c.toz = main.MorphWithTimeScale( c.toz, 0, 0.9, GSPEED );

					if ( c.y < 16 )c.toy = ( 16 - c.y ) * 0.15;

					c.toy += 1 * GSPEED;
					c.tox += ( 1 ) * dx * GSPEED;c.toz += ( 1 ) * dz * GSPEED;
		...
		var tx = c.tox * GSPEED;var ty = c.toy * GSPEED;var tz = c.toz * GSPEED;
		...
		{c.x += tx;c.y += ty;c.z += tz;
			...
			if ( dot_product < 0 ) {
				if ( c.act_jump ) if ( !c.stand ){
					if ( CanClimb( c ) ){}
					else {
						var di = main.Dist3D(-last_direction.x,-last_direction.y,-last_direction.z,
							c.walk_vector_xz.x * c.act_y - c.walk_vector_xz.y * c.act_x,0,c.walk_vector_xz.y * c.act_y + c.walk_vector_xz.x * c.act_x);
				...
				var last_tox = c.tox;var last_toy = c.toy;var last_toz = c.toz;

				c.tox = c.tox - dot_product * last_direction.x;c.toy = c.toy - dot_product * last_direction.y;c.toz = c.toz - dot_product * last_direction.z;
			...
			c.x += c.tox * GSPEED2;c.y += c.toy * GSPEED2;c.z += c.toz * GSPEED2;
			...
			while ( step_up_size > 0 ){
				...
				if ( morph2 < 1 )
				if ( morph2 > 0 ){
					c.x += last_direction.x * ( 1 - morph2 ) * step_up_size * 1;c.y += last_direction.y * ( 1 - morph2 ) * step_up_size * 1;c.z += last_direction.z * ( 1 - morph2 ) * step_up_size * 1;
					break;}
				step_up_size--;}
			...
			if ( c.sit > 0.5 )friction = sdCharacter.slide_friction;
			
			c.tox = main.MorphWithTimeScale( c.tox, 0, friction, GSPEED );c.toy = main.MorphWithTimeScale( c.toy, 0, friction, GSPEED );c.toz = main.MorphWithTimeScale( c.toz, 0, friction, GSPEED );}
				...
				if ( di < sdCharacter.player_half_width * 2 ){
					...
					if ( y_di * sdCharacter.player_half_width / sdCharacter.player_half_height * 2 > di ){
						...
						if ( c.y > c2.y ){
							...
							if ( c2.stand ) yy = c2_high;

							c.y += yy - c_low;c2.y += yy - c2_high;

							c.toy = av_toy;c2.toy = av_toy;
						...
						var xx = ( c2.x - c.x ) / di;var zz = ( c2.z - c.z ) / di;

						c.x = av_x - xx * sdCharacter.player_half_width;c.z = av_z - zz * sdCharacter.player_half_width;
						c2.x = av_x + xx * sdCharacter.player_half_width;c2.z = av_z + zz * sdCharacter.player_half_width;}}}

			//sdCharacter.player_half_height
		}

		if ( last_trace_line_percentage > 0.35 ){
			c.x = c.last_valid_x;c.y = c.last_valid_y;c.z = c.last_valid_z;
			c.sit = c.last_valid_sit;
			c.tox = 0;c.toy = 0;c.toz = 0;}

		if ( last_trace_line_percentage < 0.15 ){c.last_valid_x = c.x;c.last_valid_y = c.y;c.last_valid_z = c.z;c.last_valid_sit = c.sit;}
		...
		for ( var p = 0; p < passive_weapons.length; p++ ){
			...
			passive_weapon.position.x = 5;passive_weapon.position.y = 6;passive_weapon.position.z = 0;
			passive_weapon.rotation.y = Math.PI / 2;passive_weapon.rotation.x = Math.PI / 2;passive_weapon.rotation.z = 0;
		...
		if ( c === main.my_character ){
			...
			var front_vector = new THREE.Vector3( 0, 0, 1 );front_vector.transformDirection( main.main_camera.matrixWorld );
			var right_vector = new THREE.Vector3( 1, 0, 0 );right_vector.transformDirection( main.main_camera.matrixWorld );
			...
			main.speed.x = c.tox;main.speed.y = c.toy;main.speed.z = c.toz;
			...
			if ( sdCharacter.first_person_view ){
				...
				c.cur_weapon_mesh.position.set( 0, -3.25, -4.5 + c.recoil * main.gun_recoil );
				else{
		...
		c.arm1.quaternion.copy( c.head.quaternion );c.arm2.quaternion.copy( c.head.quaternion );
		...
		if ( c.weapon_change_tim > 0 ){
			...
			if ( c.cur_weapon_object.gun_class.old_class_id === main.WEAPON_SAW ){
				...
				if ( prog > mid )rel = ( prog - mid ) / ( 1 - mid );
				else{rel = 1 - prog / mid;rel *= rel;}
				rel = rel * 1.2 - 0.2;}
			var rot = new THREE.Quaternion();
		...
		c.leg1a.rotation.x = 0;c.leg2a.rotation.x = 0;
		
		c.leg1a.rotation.z = Math.PI * 0.5 + Math.sin( c.walk_phase ) * 0.5 - 0.25;c.leg2a.rotation.z = Math.PI * 0.5 - Math.sin( c.walk_phase ) * 0.5 - 0.25;
		c.leg1b.rotation.z = ( Math.cos( c.walk_phase - Math.PI * 0.25 ) * 0.5 + 1 ) * 1;c.leg2b.rotation.z = ( Math.cos( c.walk_phase ) * 0.5 + 1 ) * 1;
		...
		else{c.sliding_intens = main.MorphWithTimeScale( c.sliding_intens, 0, 0.9, GSPEED );}
		...
		if ( c.hurt_anim > 0 ){
			...
			for ( var i2 = 0; i2 < 4; i2++ ){
				...
				if ( i2 === 0 )m = c.head;
				if ( i2 === 1 )m = c.body;
				if ( i2 === 2 ){m = c.leg1a;scale *= -0.5;}
				if ( i2 === 3 ){m = c.leg2a;scale *= -0.5;}
				
				var power = scale * c.hurt_direction_morph;m.rotation.z += power;
				var power = scale * c.hurt_direction2_morph;m.rotation.x += power;}
			
			c.hurt_anim = Math.max( 0, c.hurt_anim - GSPEED * ( c.stand ? 0.25 : 0.05 ) );
			
			if ( c.hurt_anim === 0 ){c.hurt_direction_morph = 0;c.hurt_direction2_morph = 0;}}
		 ...
		 */
		if ( c.hook_enabled ){
			var dx = c.hook_pos.x - c.x;var dy = c.hook_pos.y - c.y;var dz = c.hook_pos.z - c.z;var di = main.Dist3D( dx, dy, dz, 0,0,0 );
			...
			if ( di > 3 ) if ( di > c.hook_di ){ c.tox += dx / di * ( di - c.hook_di ) * 0.5 * GSPEED;c.toy += dy / di * ( di - c.hook_di ) * 0.5 * GSPEED;c.toz += dz / di * ( di - c.hook_di ) * 0.5 * GSPEED;}}
		...
		if ( !main.MP_mode && c === main.my_character ){sdAI.TrainAIWithPlayerBehavior( c );}}

	static ThinkNow( GSPEED ){for ( var i = 0; i < sdCharacter.characters.length; i++ )sdCharacter.characters[ i ].UpdateCharacter( GSPEED, false );}}
