...
class sdAI{
	static init_class(){
		...
		sdAI.worker.onmessage = function(e) {
			...
			if ( command === 'status' ){main.onChatMessage( '', '...divergence: '+( data ), null, '255,255,255' );}};
		sdAI.worker.onerror = function(e) {console.log("Error: ", e);};
		...
		sdAI.brain_js_outputs = [
			'act_x','act_y',
			'act_jump','act_sit','act_sprint','act_fire','act_weapon',
	...
	constructor( c ) {
		...
		this.fire_probability = 0;this.fire_wish = 0;
	...
	static GenerateNeuralInputsForPlayerAndOpponent( c, e ) {
		...
		let dx = ( e.x - c.x ) / di;let dy = ( e.y - c.y ) / di;let dz = ( e.z - c.z ) / di;
		...
		let input = [
			dx * 0.5 + 0.5,dy * 0.5 + 0.5,dz * 0.5 + 0.5,
			
			( e.x - c.x ) / 1000 + 0.5,( e.y - c.y ) / 1000 + 0.5,( e.z - c.z ) / 1000 + 0.5,
			di / 1000,
			Math.max( 0, c.hea ) / 1000,
			Math.min( 1, c.regen_timer / 90 ),
			Math.max( 0, e.hea ) / 1000,
			Math.min( 1, e.regen_timer / 90 ),
			c.x / 1000 + 0.5,c.y / 1000 + 0.5,c.z / 1000 + 0.5,
			c.act_fire,
			e.act_fire,
			...
			e.stand * 1,
			c.stand * 1,
			e.look_direction.x * 0.5 + 0.5,e.look_direction.y * 0.5 + 0.5,e.look_direction.z * 0.5 + 0.5,
			c.look_direction.x * 0.5 + 0.5,c.look_direction.y * 0.5 + 0.5,c.look_direction.z * 0.5 + 0.5,
		];
		...
		{
			let p = ( i === 0 ) ? c : e;
			var prog_last = main.TraceLine( p.x,  p.y,  p.z, 
											 p.x + x,  p.y + y,  p.z + z, 
											 null, 3, 0 );
	...
	static TrainAIWithPlayerBehavior( c ) {
		...
		if ( sdCharacter.characters.length >= 2 ) {
			...
			if ( c !== main.my_character ) throw new Error();
			...
			for ( let i = 0; i < sdAI.brain_js_outputs.length; i++ ){
				...
				if ( parts.length === 1 ) value = c[ parts[ 0 ] ];
				else value = c[ parts[ 0 ] ][ parts[ 1 ] ];
			
				value = sdAI.brain_js_output_compress_functions[ i ]( value );
				
				if ( value < -1 || value > 1 ) throw new Error();
			...
			*/
			...
			if ( 1 ){
				...
				if ( 0 )	{
					sdAI.brain_js_model.train(
						...
						current_samples,{
							...
							*/
							iterations: 5,learningRate: sdAI.learn_speed,});	}
				...
				if ( main.ticks_passed > sdAI.next_train ){
					...
					sdAI.brain_js_model.train( 	sdAI.input_output_history,{
							//timeout: 16
							//timeout: 32,
							iterations: 1000,
							//errorThresh: 0,
				...
				//if ( main.ticks_passed > sdAI.next_train )
				{
					...
					while ( true ){
						...
						if ( dir > 0 ){
							...
							else{i2 = sdAI.input_output_history.length - 1;dir = -1;}	}
						else{
							if ( i2 + dir >= 0 ){	} else{i2 = 0;dir = 1;pong++;
				...
				function whileTraining(epoch, logs) {console.log(`Epoch: ${epoch} - loss: ${logs.loss.toFixed(2)}`);}

				function finishedTraining() {sdAI.brain_js_training_busy = false;}}}}
	
	static RunTraining(){
		if ( sdAI.input_output_history.length === 0 )return;
	...
	ApplyLogic( GSPEED ) // Never return early, because .look_direction must be normalized (or else shots will throw AI into sky)
	{
		...
		if ( sdAI.use_brain_js ){
			...
			if ( e ){
				...
				if ( sdAI.brain_js_model ){
					if ( sdAI.brain_js_model.isRunnable ) output = sdAI.brain_js_model.run( input );
					else return;}		
				else if ( sdAI.brain_js_model2 ){
					output = sdAI.brain_js_model2.activate( input );}
				else if ( sdAI.brain_js_model3 ){
			...
			*/
			...
			if ( this.control_change_tim < 0 ){
				if ( main.ai_difficulty > 0.2 ) c.act_x = ~~( Math.random() * 3 ) - 1;
				else c.act_x = 0;

				if ( Math.random() < 0.5 ) c.act_y = ~~( Math.random() * 3 ) - 1;
				else c.act_y = -1;
			...
			for ( var i = 0; i < sdCharacter.characters.length; i++ ){
				...
				if ( c.team !== c2.team ){
					...
					if ( morph === 1 )
					{
						...
						targ.x += ( c2.tox - c.tox ) / sdGunClass.weapon_speed[ this.fav_gun + ( this.fav_fire_mode - 1 ) * 2 ];targ.y += ( c2.toy - c.toy ) / sdGunClass.weapon_speed[ this.fav_gun + ( this.fav_fire_mode - 1 ) * 2 ];targ.z += ( c2.toz - c.toz ) / sdGunClass.weapon_speed[ this.fav_gun + ( this.fav_fire_mode - 1 ) * 2 ];

						var di = main.Dist3D( targ.x, targ.y, targ.z, 0, 0, 0 );
						var spread = { x:0, y:0, z:0 };
						SetAsRandom3D( spread );
						targ.x += spread.x * di * 0.3 / Math.max( 1, skill_mult );targ.y += spread.y * di * 0.3 / Math.max( 1, skill_mult );targ.z += spread.z * di * 0.3 / Math.max( 1, skill_mult );
			...
			if ( fire_prepare && !force_rocket ) this.fire_wish += this.fire_probability * GSPEED;
			...
			c.look_direction.x = this.look_vector.x;c.look_direction.y = this.look_vector.y;c.look_direction.z = this.look_vector.z;c.look_direction.normalize();}}
