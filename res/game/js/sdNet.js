...
class sdNet {
	static init_class() {
		...
		sdNet.GuessTeam = function( i, max_teams ) {
			if ( main.game_mode === main.MODE_ONE_VS_ALL ) return ( i === 0 ) ? 0 : 1;
			if ( main.MP_mode ) return ( i % max_teams );
			return ( i % max_teams ); };
		...
		function SpawnPeer() {
			...
			sdNet.peer.on('connection', function( dataConnection ){
					if ( sdNet.match_queued_dataConnections === null ) sdNet.GotNewDataConnectionAfterStart( dataConnection );
					else sdNet.match_queued_dataConnections.push( dataConnection );});
			sdNet.peer.on('disconnected', function() { setTimeout( function(){ if ( sdNet.peer.destroyed ){ SpawnPeer();} else{ sdNet.peer.reconnect();} }, 500 );});
			sdNet.peer.on('error',  function( e ) { if ( peer_give_up ) return; console.warn( e );peer_give_up = true;	});	}
...
/*	static InitiateLoginOrRegister( forceful ) {
		if ( sdNet.InitiateLoginOrRegister_block ) return;
		sdNet.InitiateLoginOrRegister_block = true;

		if ( forceful ) sdNet.uid = null;
		...
		else { sdNet.uid = ~~sdNet.uid; GetKey();}
	...
	static ResetConnection(){
		if ( main.MP_mode ){main.MP_mode = false;}
	...
	static FillWithPlayerList( e, arr, hide ){
		...
		if ( hide ){
			if ( any_not_me ){ if ( e.parentElement.style.display !== 'inherit' ) e.parentElement.style.display = 'inherit';}
			else{ if ( e.parentElement.style.display !== 'none' ) e.parentElement.style.display = 'none';}	}}
	static OpenUserMenuFor( user_uid, that ){
		...
		`;}
	static CloseUserMenu(){document.getElementById('floating_user_info_bg').style.display = 'none';}
	...
	static AcceptUsersGroup( user_uid ){
		sdNet.httpGetContent({ request:'accept_group_invite', uid:sdNet.uid, key:sdNet.key, pass_plus_key:sdNet.pass_plus_key, from_uid:user_uid }, function( v ) {
			var parts = v.split('|');
			if ( parts[ 0 ] !== 'done' ) sdNet.GotServerError( v );
		}, true );}
	...
	static UpdateGroupPlayers(){
		sdNet.httpGetContent({ request:'get_group_players', uid:sdNet.uid, key:sdNet.key, pass_plus_key:sdNet.pass_plus_key }, function( v ){
			...
			if ( parts[ 1 ].indexOf( '>' ) !== -1 ){
				if ( !sdNet.invited_to_group ){ sdNet.invited_to_group = true; sdSound.PlayInterfaceSound({ sound: lib.ui_notification, volume: 1 });}}
			else{sdNet.invited_to_group = false;}
	...
	static UpdateEnemyGroupPlayers(){
		sdNet.httpGetContent({ request:'get_enemy_group_players', uid:sdNet.uid, key:sdNet.key, pass_plus_key:sdNet.pass_plus_key }, function( v ){
			...
			else{sdNet.invited_to_enemy_group = false;}
	...
	static httpGetContent( param_obj, callback, retry ){
		...
		if ( sdNet.request_stack[ i ][ 0 ].request === param_obj.request ){return;}
	...
	static _httpGetContent( param_obj, callback, retry ){
		if ( retry === true ) retry = true; else retry = false;
		...
		else theUrl = 'http://www.gevanni.com/projects/StarDefenders3D/server.php';
	...
	static ConnectionError( n, t ){ if ( n === 0 ){ t = 'Connection lost'; } }
	static OfflineTraining( enemies ){
		...
		if ( enemies === 2 ){ max_players = 16; mode = main.MODE_ONE_VS_ALL; }
		if ( enemies === 0 ){ max_players = 1; }
		if ( enemies === 3 ) // Mimicking AI
		{ max_players = 2; sdAI.use_brain_js = true; }
		...
		function then(){ main.MP_mode = false; main.run(); } }
	...
	static PeersAddStart(){console.log( 'PeersAddStart' );sdNet.peer_counter = 0;}
	...
	static PeersAddEnd() {
		...
		if ( sdNet.peer_counter <= 1 ) throw new Error('Why there is only '+sdNet.peer_counter+' peers?');}
	static StartMatch( params, then=null ) { /* .max_players .max_teams .world_seed .map_uid .mode */
		...
		function Retry_function() {
			...
			main.game_mode = params.mode; main.BuildLevel( Number( params.world_seed ) );
			...
			main.team_scores = [];
			for ( var i = 0; i < max_teams; i++ ) main.team_scores[ i ] = 0;
			...
			for ( var i = 0; i < sdNet.match_queued_dataConnections.length; i++ ) sdNet.GotNewDataConnectionAfterStart( sdNet.match_queued_dataConnections[ i ] );
