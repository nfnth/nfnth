...
class main {
	...
	static get sound_volume(){return sdSound.MASTER_SOUND_VOLUME;}
	static set sound_volume( v ){sdSound.MASTER_SOUND_VOLUME = v;}
	static get music_volume(){return main._music_volume;}
	static set music_volume( v ){ main._music_volume = v; main.PlaySongIfNeeded(); sdSound.SetSoundVolume( main.song_channel, v );}
	static get first_person_camera(){return sdCharacter.first_person_view ? 1 : 0;}
	...
	static init_class(){
		...
		if ( main.sensitivity === null ) main.sensitivity = 0.0016; // 0.0015
		else main.sensitivity = Number( main.sensitivity );
		
		/* 0 = freely, 1 = xy mouse, 2 = device orientation */
		main.turn_method = localStorage.getItem('stardefenders_turn_method');
		if ( main.turn_method === null ) main.turn_method = 1;
		else main.turn_method = Number( main.turn_method );
		...
		setTimeout( function(){
			...
			document.getElementById('low_physics').value = main.low_physics;}, 1000 );
		
		main.ang = 0; main.ang2 = 0; main.ang3 = 0;
		...
		setInterval(
		function(){
			if ( chat_box === null )chat_box = document.getElementById('chat_box');
			
			if ( main.chat_off_timer > 0 ){
				main.chat_off_timer -= 0.02;
				
				if ( main.chat_off_timer <= 0 ) main.chat_messages = [];
				
				if ( main.chat_off_timer < 1 ) chat_box.style.opacity = main.chat_off_timer;
				else chat_box.style.opacity = '1';}}, 20 );
		...
		mini_texture.onload = function() {bmp.ctx.drawImage( mini_texture, 0, 0 );};
		...
		main.songs = ['maD__Alg0rh1tm_-_02_-_alg0rh1tm_-_Sandglass','maD__Alg0rh1tm_-_01_-_alg0rh1tm_-_Circuit'];
		main.song_titles = ['alg0rh1tm - Sandglass by maD & Alg0rh1tm','alg0rh1tm - Circuit by maD & Alg0rh1tm'];
	...
	static ScoreForTeam( team_id, delta ){
		...
		for ( var i = 0; i < main.team_scores.length; i++ ){
			...
			a.r = a.r * 0.7 + 0.3;a.g = a.g * 0.7 + 0.3;a.b = a.b * 0.7 + 0.3;
			a.r = ~~( a.r * 255 );a.g = ~~( a.g * 255 );a.b = ~~( a.b * 255 );
	...
	static InitEngine() {
		...
		main.main_camera.position.x = -5;main.main_camera.position.y = -5;main.main_camera.position.z = -5;
		...
		window.ondeviceorientation = function( e ){
			...
			// Lag prevention
			{sdGunClass.weapon_hp_damage[ 2 ] *= 3;sdGunClass.weapon_hp_damage_head[ 2 ] *= 3;sdGunClass.weapon_knock_count[ 2 ] /= 3;}
			...
			setTimeout( function(){
				...
				}, function() { main.hold_w = 0; main.hold_d = 0; });
			...
			function start_handler( e ){
				if ( document.getElementById( 'ingame_hud' ).style.display === 'none' )return;
			...
			function end_handler( e ) {
				for ( var i = 0; i < buttons.length; i++ ){
					if ( e.changedTouches[ 0 ].identifier === buttons[ i ].touch_id ){
						...
						if ( buttons[ i ].action_release !== null ) buttons[ i ].action_release();
						break;}}	}
	...
	static onChatMessage( from, t, character=null, color='255,255,255' ){
		...
		if ( main.chat_messages.length > 5 ){main.chat_messages.shift();}
		
		var s = '';
		
		for ( var i = 0; i < main.chat_messages.length; i++ ){s += main.chat_messages[ i ];}
	...
	static onKeyDown( e ){
		...
		if ( document.activeElement.tagName === 'INPUT' && !document.activeElement.isContentEditable ){return true;}
		...
		if ( document.activeElement.isContentEditable ){
			if ( e.keyCode === 13 ) // Enter
			{
				...
				if ( document.getElementById('chat_input_box').value.length > 0 ){
					if ( main.MP_mode ) sdSync.MP_SendEvent( sdSync.COMMAND_I_SAY, document.getElementById('chat_input_box').value );
		...
		else if ( e.keyCode === 192 || e.keyCode === 48 ) // ~ or 0
		...
		if ( e.keyCode === 90 ) // Z
		{
			if ( !main.MP_mode ){main.GAME_FPS = ( main.GAME_FPS === 15 ) ? 30 : 15;}}
	...
	static OpenEscMenu(){
		if ( document.getElementById('menu').style.display !== 'inherit' ){
			...
			function Fractally( e ){
				if ( e.type === 'range' )main.ShowSliderNumber( e, 0 );
	...
	static SetActiveCharacter( c ){
		if ( c === main.my_character ) return;
	
		if ( main.my_character !== null ) main.my_character.UnhideForFPS();
	...
	static onMouseDown( e ){
		...
		if ( main.my_character.hea > 0 ){
			if ( main.my_character.hook_enabled ){
				...
				else sdSound.PlaySound({ sound: lib.blocked_damage, parent_mesh: main.my_character.mesh, volume: 1, pitch: 0.5 });}}
		
		if ( e.which === 1 ) main.hold_fire = 1;
	
		if ( e.which === 3 ) main.zoom_intensity_target = 0.5;
		//main.hold_fire = 2;
	}
	static onMouseUp( e ){ if ( e.which === 3 ) main.zoom_intensity_target = 1; else main.hold_fire = 0; }
	...
	static GoFullscreen(){
		...
		if ( document.pointerLockElement !== main.renderer.domElement ) main.renderer.domElement.requestPointerLock();}
	...
	static Dist3D_Vector_pow2( tox, toy, toz ){return ( tox*tox + toy*toy + toz*toz );}
	static Dist3D( x1, y1, z1, x2, y2, z2 ){return Math.sqrt( main.Dist3D_Vector_pow2( x1-x2, y1-y2, z1-z2 ) );}
	...
	static BuildLevel( seed ){
		...
		function AllowLevelBuildingAt( x, y, z, initial ){
			var max_x = size_x / 32 - 1;var max_y = size_y / block_height - 1;var max_z = size_z / 32 - 1;
			
			if ( y === max_y )return false;
		
			if ( x < 5 )return true;
		
			if ( x >= level_chunks_x - 1 - 5 )return true;
		
			if ( y < max_y - 2 )
			if ( z > 0 && z < max_z ){return true;}
		...
		main.level_chunks_x = level_chunks_x;main.level_chunks_y = level_chunks_y;main.level_chunks_z = level_chunks_z;
		...
		function Coord( x, y, z ){return ( x * size_z_mul_size_y + y * size_z + z );}
		
		function FromCoord( x,y,z ){return reusable_coords[ Coord( x,y,z ) ];}
		...
		function SolveBasedOn( solvable, arr, size ){
			if ( solved[ solvable.coord ] )return;

			var sum = 0;var noise = 0;
			for ( var k = 0; k < arr.length; k++ ){sum += fill[ arr[ k ] ];noise += noise_tex[ arr[ k ] ];}

			sum /= arr.length;noise /= arr.length;
			...
			if ( all_solvable_based_ons_x[ solvable.x ] === undefined ) all_solvable_based_ons_x[ solvable.x ] = 1;
			else all_solvable_based_ons_x[ solvable.x ]++;
	
			if ( solvable.y === 0 ){sum = Math.max( edge_density + 0.02, sum );}

			if ( completely_random_percentage === 0 ){if ( solvable.y === size_y-1 ){sum = Math.min( edge_density - 0.02, sum );}}
		...
		function SolveIfNotSolved( i, val ){
			if ( solved[ i ] ) return;
			fill[ i ] = val;noise_tex[ i ] = val;solved[ i ] = true;}
		...
		while ( c.g * 0.5 > c.r + c.b ){c.g -= 0.05;c.r += 0.1;c.b += 0.1;}
		while ( c.r * 0.45 > c.g + c.b ){c.r -= 0.05;c.g += 0.1;c.b += 0.1;}
		while ( c.b * 0.35 > c.r + c.g ){c.b -= 0.05;c.r += 0.075;c.g += 0.075;}
		main.fog_color_color = c;main.fog_color = c._uint;
		var terrain_color = new pb2HighRangeColor().rand_pattern();
		main.renderer.setClearColor( main.fog_color );

		sdAtom.RandomizeStars();
		
		if ( main.ground_mesh ){main.DestroyMovieClip( main.ground_mesh );}
		...
		m.depthWrite = true;
		m.transparent = false;
		m.uniforms.fog.value = new THREE.Color( main.fog_color );m.uniforms.fog_intensity.value = 1;
		m.uniforms.brightness_r.value = 1;m.uniforms.brightness_g.value = 1;m.uniforms.brightness_b.value = 1;
		main.ground_mesh = new THREE.Mesh( g, m );
		main.ground_mesh.rotation.x = -Math.PI * 0.5;
		main.ground_mesh.position.x = size_x / 2;main.ground_mesh.position.z = size_z / 2;main.ground_mesh.position.y = main.world_end_y;
		...
		if ( add_terrain ){
			...
			for ( z = 0; z < size_z; z += step_size ){
				PreSolveCube( x, y,z, Math.min( x + step_size, size_x-1 ),Math.min( y + step_size, size_y-1 ),Math.min( z + step_size, size_z-1 ) );
				SolveCube( x, y, z, Math.min( x + step_size, size_x-1 ), Math.min( y + step_size, size_y-1 ),Math.min( z + step_size, size_z-1 ) );}}
		else{
		...
		if ( size_x % 32 !== 0 )throw new Error('Size X is not supported by level builder');
		if ( size_y % block_height !== 0 )throw new Error('Size Y is not supported by level builder');
		if ( size_z % 32 !== 0 )throw new Error('Size Z is not supported by level builder');
		
		class Entry{constructor( v=false ){this.is_wall = v;this.is_stairs = false;this.is_door = false;this.is_lamp = false;}	}
		...
		for ( var x = 0; x < size_x / 32; x++ ){
			...
			for ( var y = 0; y < size_y / block_height; y++ ){
				level_grid[ x ][ y ] = [];level_grid[ x ][ y ].length = size_z / 32;
		...
		for ( var z = 0; z < level_grid[ x ][ y ].length; z++ ){
			...
			else if ( AllowLevelBuildingAt( x, y, z, false ) ) // No highest level or else borders won't be made
			{
				// Prevent some open ceilings
				if ( !WallExists( x, y-1, z ) && !WallExists( x, y-2, z ) ){
					if ( sdRandomPattern.random() < 0.2 )level_grid[ x ][ y ][ z ].is_wall = true;}
				else{
		...
		var MAT_WALL = 0;var MAT_DOOR = 1;var MAT_STAIRS = 2;var MAT_BORDER = 3;var MAT_LAMP = 4;
		...
		function WallExists( x, y, z ){
			...
			if ( x >= level_grid.length )return false;
			if ( y >= level_grid[ x ].length )return false;
			if ( z >= level_grid[ x ][ y ].length )return false;
		...
		function GetStairsAt( x, y, z ){
			...
			if ( x >= level_grid.length )return null;
			if ( y >= level_grid[ x ].length )return null;
			if ( z >= level_grid[ x ][ y ].length )return null;
		
			if ( level_grid[ x ][ y ][ z ].is_stairs )return level_grid[ x ][ y ][ z ];
		...
		for ( var z = 0; z < level_grid[ x ][ y ].length; z++ ){
			if ( level_grid[ x ][ y ][ z ].is_wall ){
				var x1 = x * 32;var x2 = x1 + 32;
				var y1 = y * block_height;var y2 = y1 + block_height;
				var z1 = z * 32;var z2 = z1 + 32;
				...
				if ( !LampExists( x, y + 1, z ) ){
					if ( WallExists( x - 1, y, z ) && WallExists( x + 1, y, z ) && !WallExists( x, y, z - 1 ) && !WallExists( x, y, z + 1 ) &&
						...
						var x1 = x * 32;var x2 = x1 + 32;
						var y1 = y * block_height;var y2 = y1 + 32;
						var z1 = z * 32;var z2 = z1 + 32;
						
						var cz = ( z1 + z2 ) / 2;z1 = ~~( cz - 3 );z2 = ~~( cz + 3 );
						
						cut_ops.push({ mat:MAT_DOOR, orientation:1, x1:x1, y1:y1, z1:z1, x2:x2, y2:y2, z2:z2 });
						level_grid[ x ][ y ][ z ].is_door = true;
						continue;}
					else if ( !WallExists( x - 1, y, z ) && !WallExists( x + 1, y, z ) && WallExists( x, y, z - 1 ) && WallExists( x, y, z + 1 ) &&
						 !WallExists( x - 1, y + 1, z ) && !WallExists( x + 1, y + 1, z ) && WallExists( x, y + 1, z - 1 ) && WallExists( x, y + 1, z + 1 ) ){
						var x1 = x * 32;var x2 = x1 + 32;
						var y1 = y * block_height;var y2 = y1 + 32;
						var z1 = z * 32;var z2 = z1 + 32;
				...
				if ( WallExists( x, y+1, z ) ){
					...
					if ( sdRandomPattern.random() < 0.5 ){
						var cx = ( x1 + x2 ) / 2;x1 = ~~( cx - 1 );x2 = ~~( cx + 1 );
						var cz = ( z1 + z2 ) / 2;z1 = ~~( cz - 1 );z2 = ~~( cz + 1 );
						...
						var cx = ( x1 + x2 ) / 2;x1 = ~~( cx - 3 );x2 = ~~( cx + 3 );
						var cz = ( z1 + z2 ) / 2;z1 = ~~( cz - 3 );z2 = ~~( cz + 3 );
				...
				if ( WallExists( x, y-1, z ) ) // Anything under
				{
					...
					if ( level_grid[ x ][ y ][ z ].is_stairs )continue;}}}
		...
		if ( sdRandomPattern.random() < 0.25 ){
			var x1 = cut_ops[ i ].x1;var x2 = cut_ops[ i ].x2;
			var y1 = cut_ops[ i ].y1 + 2;var y2 = cut_ops[ i ].y2 - 2;
			var z1 = cut_ops[ i ].z1;var z2 = cut_ops[ i ].z2;

			var cx = ( x2 + x1 ) / 2;
			var cz = ( z2 + z1 ) / 2;
			
			if ( x2 - x1 > z2 - z1 ){x1 = cx - 1;x2 = cx + 1;}
			else{z1 = cz - 1;z2 = cz + 1;}
		...
		var level_bitmap_pixels = [];level_bitmap_pixels.length = main.level_bitmap.width;
		for ( var x = 0; x < main.level_bitmap.width; x++ ){
			...
			for ( var y = 0; y < main.level_bitmap.height; y++ ){
				...
				var rgb_unique = new sdRGB();rgb_unique.r = rgba.r;rgb_unique.g = rgba.g;rgb_unique.b = rgba.b;
				level_bitmap_pixels[ x ][ y ] = rgb_unique;}}
		function GetPixelFromLevelBitmap( x, y ){return level_bitmap_pixels[ x ][ y ];}
		...
		for ( z = 0; z < size_z; z++ ){
			...
			if ( test_trace_setup ){
				if ( y < 5 ){
					world_rgb[ c++ ] = terrain_color.r * 255;world_rgb[ c++ ] = terrain_color.g * 255;world_rgb[ c++ ] = terrain_color.b * 255;
					fill[ i ] = 1;}
				else{
					if ( Math.max( x, z ) % 40 <= 0 ){
						world_rgb[ c++ ] = 100;world_rgb[ c++ ] = 100;world_rgb[ c++ ] = 100;
						fill[ i ] = 1;}
					else{
						fill[ i ] = 0;
						c += 3;}}
			...
			if ( fill[ i ] > 0 ){
				if ( !add_concrete ){
					...
					world_rgb[ c++ ] = terrain_color.r * 255;world_rgb[ c++ ] = terrain_color.g * 255;world_rgb[ c++ ] = terrain_color.b * 255;
					continue;}
				world_rgb[ c   ] = terrain_color.r * 255;world_rgb[ c+1 ] = terrain_color.g * 255;world_rgb[ c+2 ] = terrain_color.b * 255;}
			c += 3;}
		// Other way
		//if ( false )
		{
				...
				for ( z = cut_ops[ op ].z1; z < cut_ops[ op ].z2; z++ ){
					...
					if ( cut_ops[ op ].mat === MAT_WALL ){
						...
						if ( on_border || on_deeper ){
							...
							var off_x = 32; var off_y = 0;
							rgba = GetPixelFromLevelBitmap( ( x % 32 ) + off_x, ( z % 32 ) + off_y );}}
					else if ( cut_ops[ op ].mat === MAT_DOOR ) {
						var off_x = 0; var off_y = 32;
						...
						if ( cut_ops[ op ].orientation === 0 ){
							...
							else // Cap
							{
								...
								var off_y = 32;
								rgba = GetPixelFromLevelBitmap( x % 32 + off_x, z % 32 + off_y );}}}
					else if ( cut_ops[ op ].mat === MAT_STAIRS ){
						...
						if ( cut_ops[ op ].orientation === 0 ){elevation = ( x - cut_ops[ op ].x1 ) * 0.75 - 8;}
						else if ( cut_ops[ op ].orientation === 1 ){elevation = -( x - cut_ops[ op ].x2 + 1 ) * 0.75 - 8;}
						else if ( cut_ops[ op ].orientation === 2 ){elevation = ( z - cut_ops[ op ].z1 ) * 0.75 - 8;}
						else if ( cut_ops[ op ].orientation === 3 ){elevation = -( z - cut_ops[ op ].z2 + 1 ) * 0.75 - 8;}
					...
					else if ( cut_ops[ op ].mat === MAT_BORDER ){
						var off_x = 32;var off_y = 32;
						if ( cut_ops[ op ].orientation === 0 ) rgba = GetPixelFromLevelBitmap( ~~((y-cut_ops[ op ].y2+34)*0.25) % 32 + off_x, ~~(z*0.25) % 32 + off_y );
						else rgba = GetPixelFromLevelBitmap( ~~((y-cut_ops[ op ].y2+34)*0.25) % 32 + off_x, ~~(x*0.25) % 32 + off_y );
					
						if ( rgba.r < 30 )fill_power = -10;}
					else if ( cut_ops[ op ].mat === MAT_LAMP ){rgba = reusable_rgba;rgba.r = 255;rgba.g = 255;rgba.b = 255;noise_tex[ i ] = 1;}

					if ( rgba !== null ){world_rgb[ c   ] = rgba.r;world_rgb[ c+1 ] = rgba.g;world_rgb[ c+2 ] = rgba.b;}
		...
		for ( z = 1; z < size_z-1; z++ ){
			...
			if ( cur_fill ){
				...
				if ( filled_around === 6 ){brightness[ i ] = -1; // Will mean it is invisible
					}}}
		...
		main.lightmap_ambient *= global_brightness;main.lightmap_beam_power *= global_brightness;
		...
		for ( z = 0; z < size_z; z++ ){
			...
			if ( fill[ i ] > edge_density ){
				...
				if ( xx !== 0 || zz !== 0 ){
					...
					if ( xx > 0 )till_sun = Math.min( till_sun, ( size_x - x - 1 ) * 2 );
					else if ( xx < 0 )till_sun = Math.min( till_sun, ( x - 1 ) * 2 );
			
					if ( zz > 0 )till_sun = Math.min( till_sun, ( size_z - z - 1 ) * 2 );
					else if ( zz < 0 )till_sun = Math.min( till_sun, ( z - 1 ) * 2 );
					...
					while ( till_sun > 0 ){
						...
						if ( rare <= 0 ){i2 += rare_speed;rare += rare_scale;}
						rare--;
		...
		if ( main.isWinter ){
			...
			for ( z = 0; z < size_z; z++ ){
				...
				for ( y = size_y - 1; y >= 0; y-- ){
					...
					if ( brightness[ i ] !== -1 )
					if ( fill[ i ] > edge_density ){
						var c = i * 3;world_rgb[ c   ] = 255;world_rgb[ c+1 ] = 255;world_rgb[ c+2 ] = 255;
		...
		for ( var p = 0; p < sdLamp.lamps.length; p++ ){
			...
			var x1 = m.minx = Math.max( m.minx, 0 );var y1 = m.miny = Math.max( m.miny, 0 );var z1 = m.minz = Math.max( m.minz, 0 );
			
			var x2 = m.maxx = Math.min( m.maxx, size_x );var y2 = m.maxy = Math.min( m.maxy, size_y );var z2 = m.maxz = Math.min( m.maxz, size_z );
			...
			for ( z = z1; z < z2; z++ ){
				...
				if ( fill[ i ] > edge_density ){
					...
					if ( di < m.glow_radius ){
						if ( di < m.collision_radius ) brightness[ i ] += 2;
						else brightness[ i ] += ( 1 - di / m.glow_radius ) * 2;}}}}
		...
		for ( var zz = 0; zz < level_chunks_z; zz++ ){
			...
			var v = 0;var c = 0;var b = 0;
			for ( var x = 0; x < chunk_size; x++ )
			for ( var y = 0; y < chunk_size; y++ )
			for ( var z = 0; z < chunk_size; z++ ){
				var i = Coord( xx * chunk_size + x, yy * chunk_size + y, zz * chunk_size + z );
				
				vertices[ v++ ] = xx * chunk_size + x;vertices[ v++ ] = yy * chunk_size + y;vertices[ v++ ] = zz * chunk_size + z;
				
				var br = noise_tex[ i ];
				rgba[ c++ ] = world_rgb[ i*3   ] / 255 * br;rgba[ c++ ] = world_rgb[ i*3+1 ] / 255 * br;rgba[ c++ ] = world_rgb[ i*3+2 ] / 255 * br;
				
				
				if ( fill[ i ] > edge_density ){rgba[ c++ ] = 1; // Alpha
					dots_total++;}
				else rgba[ c++ ] = 0; // Alpha
			...
			chunk.rgba = rgba;chunk.uvs2 = uvs2;
			...
			if ( zz > 0 ){chunk.Connect( main.voxel_static[ main.voxel_static.length - 1 ] );}
			if ( yy > 0 ){chunk.Connect( main.voxel_static[ main.voxel_static.length - level_chunks_z ] );}
			if ( xx > 0 ){chunk.Connect( main.voxel_static[ main.voxel_static.length - level_chunks_z * level_chunks_y ] );}
	...
	static DestroyMovieClip( mc, keep_material=false ){
		while ( mc.children.length > 0 ){main.DestroyMovieClip( mc.children[ mc.children.length - 1 ], keep_material ); }
		...
		if ( mc.isAudio ){mc.remove();}
		if ( mc.parent !== null )mc.parent.remove( mc );}
	...
	static WorldPaintDamage( x, y, z, radius, mode=0/*is_gore_painter=false*/,red=0,green=0,blue=0 ) // mode: cutter=0, gore_paiter=1, builder=2
	{
		...
		if ( mode === MODE_CUTTER )radius_int += 2;
	
		if ( mode === MODE_GORE_PAINTER )radius_int += 2; // Not sure about this one
		...
		function chunk_update_function( chunk ){
			if ( chunk_updates.indexOf( chunk ) === -1 )chunk_updates.push( chunk );}
		...
		// Smallest voxel
		var av_x = ~~( x );var av_y = ~~( y );var av_z = ~~( z );

		var chunk_xx_min = ~~( ( av_x - radius_int ) / chunk_size );var chunk_yy_min = ~~( ( av_y - radius_int ) / chunk_size );var chunk_zz_min = ~~( ( av_z - radius_int ) / chunk_size );
		...
		if ( chunk_xx_min < 0 )chunk_xx_min = 0;
		if ( chunk_yy_min < 0 )chunk_yy_min = 0;
		if ( chunk_zz_min < 0 )chunk_zz_min = 0;
		if ( chunk_xx_max > main.level_chunks_x - 1 )chunk_xx_max = main.level_chunks_x - 1;
		if ( chunk_yy_max > main.level_chunks_y - 1 )chunk_yy_max = main.level_chunks_y - 1;
		if ( chunk_zz_max > main.level_chunks_z - 1 )chunk_zz_max = main.level_chunks_z - 1;
		...
		for ( var chunk_zz = chunk_zz_min; chunk_zz <= chunk_zz_max; chunk_zz++ ){
			...
			av_x = ~~x - chunk_xx * chunk_size;av_y = ~~y - chunk_yy * chunk_size;av_z = ~~z - chunk_zz * chunk_size;
			...
			for ( var zz = radius_int_z_min; zz < radius_int_z_max; zz++ )
			{
				...
				if ( di < radius_pow2 ){
					...
					if ( chunk.rgba[ i * 4 + 3 ] > 0 ){
						...
						chunk.dots_total++;
						chunk.rgba[ i * 4 + 0 ] = red;chunk.rgba[ i * 4 + 1 ] = green;chunk.rgba[ i * 4 + 2 ] = blue;
						chunk.rgba[ i * 4 + 3 ] = 1;
						
						if ( di >= radius - 1 ) chunk.uvs2[ i ] = Math.random();
						else chunk.uvs2[ i ] = -1;	
				...
				if ( di < radius_pow2_vis_upd ){
					if ( chunk.uvs2[ i ] < 0 )chunk.uvs2[ i ] = Math.random();
					...
					if ( morph < 1 ){
						...
						chunk.rgba[ i * 4 ] *= morph;chunk.rgba[ i * 4 + 1 ] *= morph;chunk.rgba[ i * 4 + 2 ] *= morph;}}}
		...
		if ( anything_done ){
			if ( mode === MODE_GORE_PAINTER ){
				...
				if ( geometry_changed ){main.RecalcBrightness( ~~x, ~~y, ~~z, radius_int + 1, chunk_update_function );}

				for ( var i = 0; i < chunk_updates.length; i++ ) chunk_updates[ i ].UpdateChunk( false, true, true );}}
	...
	static SetAsRandom3D( v ){
		...
		v.x = one_minus_sqr_z * Math.cos(omega);v.y = one_minus_sqr_z * Math.sin(omega);v.z = z;

		return v;}
	
	static GetEntityBrightness( x, y, z ){
		if ( main.mobile )return 1;
		...
		if ( Math.abs( main.GetEntityBrightness_lz - z ) <= cache_size ){return main.GetEntityBrightness_lr;}
		...
		function IncludeLamps( brightness ){
			for ( var p = 0; p < sdLamp.lamps.length; p++ ){
				...
				if ( z > m.z - m.glow_radius && z < m.z + m.glow_radius ){{
						...
						if ( y < m.y )di = main.Dist3D_Vector_pow2( m.x-x,(m.y-y)*m.any_distance_y_scale,m.z-z );
						else di = main.Dist3D_Vector_pow2( m.x-x,(m.y-y)*m.any_distance_y_scale_above,m.z-z );
		...
		var size_x = main.level_chunks_x * chunk_size;var size_y = main.level_chunks_y * chunk_size;var size_z = main.level_chunks_z * chunk_size;
		
		x = ~~x;y = ~~y;z = ~~z;
		
		if ( x < 0 || y < 0 || z < 0 )return IncludeLamps( main.lightmap_ambient + main.lightmap_rays_per_direction * main.lightmap_beam_power * 4 );
	
		if ( x > size_x-1 || y > size_y-1 || z > size_z-1 )return IncludeLamps( main.lightmap_ambient + main.lightmap_rays_per_direction * main.lightmap_beam_power * 4 );
		
		var chunk_xx = ~~( x / chunk_size );var chunk_yy = ~~( y / chunk_size );var chunk_zz = ~~( z / chunk_size );

		var ch = chunk_xx * main.level_chunks_y * main.level_chunks_z + chunk_yy * main.level_chunks_z + chunk_zz;

		var chunk = main.voxel_static[ ch ];

		var av_x = x - chunk_xx * chunk_size;var av_y = y - chunk_yy * chunk_size;var av_z = z - chunk_zz * chunk_size;
		...
		var brightness = 0;{
			...
			if ( xx !== 0 || zz !== 0 ){
				...
				var current_av_x = av_x;var current_av_y = av_y;var current_av_z = av_z;
				...
				while ( till_sun > 0 ){
					...
					if ( rare <= 0 ){
						...
						current_av_x += xx;current_av_z += zz;
	...
	static RecalcBrightness( ux, uy, uz, radius_int=Infinity ) {
		...
		for ( var i = 0; i < main.recalc_brightness_tasks.length; i++ ){
			...
			if ( Math.abs( uz - arr2[ 2 ] ) <= radius_int + arr2[ 3 ] ){if ( arr2[ 3 ] < radius_int ){insert_at = i;break;}}}
		
		main.recalc_brightness_tasks.splice( insert_at, 0, arr );
		main.recalc_brightness_hash++;}
	
	static _RecalcBrightness( arr ) {
		var ux = arr[ 0 ];var uy = arr[ 1 ];var uz = arr[ 2 ];
		...
		if ( radius_int === Infinity )highest_y = size_y;
		
		var from_x = 0;var from_z = 0;
		var to_x = size_x;var to_z = size_z;
		...
		for ( y; y >= 0; y-- ){
			...
			y_steps_to_do--;
			if ( y_steps_to_do < 0 ){SaveVariables(); return false;}
			
			y_rare_spread_inc++;
			
			if ( y_rare_spread_inc >= 3 ){
				y_rare_spread_inc = 0;
				if ( from_x > 0 )from_x -= 1;
				if ( from_z > 0 )from_z -= 1;
				if ( to_x < size_x )to_x += 1;
				if ( to_z < size_z )to_z += 1;}
				...
				if ( chunk.rgba[ i * 4 + 3 ] > 0 ){
					...
					if ( chunk.recalc_brightness_current_update_hash[ i ] < main.recalc_brightness_hash ){
						...
						for ( var p = 0; p < sdLamp.lamps.length; p++ ){
							...
							var x1 = m.minx;var y1 = m.miny;var z1 = m.minz;

							var x2 = m.maxx;var y2 = m.maxy;var z2 = m.maxz;
	...
	static TraceLine( x,y,z, x2,y2,z2, stop_condition_chunk=null, step_scale=1, offset=0 ) // First point and last point, not relative to each other.
	{
		...
		function CheckBounds( x3, y3, z3, needs_round ){
			...
			if ( x3 >= main_level_chunks_x_mul_chunk_size )return false;
			if ( y3 >= main_level_chunks_y_mul_chunk_size )return false;
