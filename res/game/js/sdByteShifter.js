...
class sdByteShifter{
	static init_class(){
		sdByteShifter.message_id_delimeter = ':';sdByteShifter.message_delimeter = ';';sdByteShifter.command_delimeter = '|';
		...
		{
			if ( for_dataConnection.character.team === main.my_character.team ) return true;
			if ( for_dataConnection.reports_direct_visiblity ) return true;
			...
			if ( for_dataConnection.visible_chunks.indexOf( ch ) !== -1 ) return true;
	...
	ListIDsWithRanges( arr ){
		...
		for ( var i = 0; i < arr.length; i++ ){
			...
			for ( var i2 = i + 1; i2 < arr.length; i2++ ) if ( arr[ i2 ] !== arr[ i2 - 1 ] + 1 ) break;
			i2--;
			
			if ( i2 > i ) {r += '+' + ( i2 - i );i = i2;continue;}}
		return r;}
	IDsWithRangesToArray( s ){
		...
		for ( var i = 0; i < parts.length; i++ ){
			...
			if ( parts2.length === 1 ) arr.push( initial_num );
			else{
				// range
				var extra = Number( parts2[ 1 ] ) ;
				for ( var i2 = 0; i2 <= extra; i2++ ) arr.push( initial_num + i2 );}}
	...
	SendData( GSPEED ){
		...
		for ( var i = 0; i < this.received_ids_storage.length; i++ ){
			...
			if ( this.received_ids_storage[ i ].timeout_in < 0 ){main.RemoveElement( this.received_ids_storage, i );i--;continue;}}
		...
		if ( sdByteShifter.debug_sent_received_data ){
			console.log('SENT-LZW: '+data_LZW );console.log('SENT: '+data);
			var s = '';
			for ( var i = 0; i < data_LZW.length; i++ ){s += data_LZW.charCodeAt( i )+' / ';}
			console.log( s );}}
	...
	TryDecode( s ){
		...
		while ( true ){
			...
			var relevant_part = s.slice( offset + 1 );var parts = relevant_part.split('^');
			var i = Number( parts[ 0 ] );var x = Number( parts[ 1 ] );var w = Number( parts[ 2 ] );
			
			// #i^x^w^37|0.34|0.86|-0.4|-0.92|0|0|0|0|0|0|0|0|3.14
			
			var pos_x = s.indexOf( '^', offset+1 );
			if ( pos_x === -1 ) throw new Error('Corrupted packet?');
		
			var pos_w = s.indexOf( '^', pos_x+1 );
			if ( pos_w === -1 ) throw new Error('Corrupted packet?');
		
			var pos_end = s.indexOf( '^', pos_w+1 );
			if ( pos_end === -1 ) throw new Error('Corrupted packet?');
	...
	SendArray( arr, GSPEED ){
		...
		if ( !arr[ i ].is_sent ){
			if ( arr[ i ].self_sent_in > 0 ){
				...
				if ( arr[ i ].self_sent_in <= 0 ){arr[ i ].is_sent = true;arr[ i ].self_sent = true;}}
			else{arr[ i ].is_sent = true;arr[ i ].self_sent = true;}
	...
	DataReceived( data_LZW){
		...
		if ( sdByteShifter.debug_sent_received_data ){console.log('RECEIVED-LZW: '+data_LZW );console.log('RECEIVED: '+data );}
		...
		for ( var i = 0; i < messages.length; i++ ){
			...
			var parts = messages[ i ].split( sdByteShifter.message_id_delimeter ); var event_id = parts[ 0 ];
			...
			if ( event_id === 'rcvs' ){
				if ( parts[ 1 ].length > 0 ){
					...
					for ( var i2 = 0; i2 < received_ids.length; i2++ ){
						...
						if ( this.stack_any[ id ] === undefined ) throw new Error('rcvs value sent by peer makes no sense');

						this.stack_any[ id ].is_sent = true; this.stack_any[ id ].self_sent = false;	}}}
			else{
				var id = ~~parts[ 0 ];
				// Anti-duplicate measure
				var ok = true;
				for ( var i2 = 0; i2 < this.received_ids_storage.length; i2++ )
				if ( this.received_ids_storage[ i2 ].id === id ){ok = false;break;}
				if ( ok ){parts[ 1 ] = this.TryDecode( parts[ 1 ] );
					if ( this.DecodeCommand( parts[ 1 ] ) ){this.received_ids.push( id );this.received_ids_storage.push( new ReceivedID( id ) );this.received_messages[ id ] = parts[ 1 ];}}}	}}
	static approx( x ){return Math.round( x * 100 ) / 100;}
	approx( x ){return sdByteShifter.approx( x );}
	AddEncodeCommand( command, ... args ){
		...
		if ( command === sdSync.COMMAND_CHARACTER_STATE ){
			...
			message = 
				command + _ + 
				this.approx( c.x ) + _ + this.approx( c.y ) + _ + this.approx( c.z ) + _ + 
				this.approx( c.tox ) + _ + this.approx( c.toy ) + _ + this.approx( c.toz ) + _ + 
				this.approx( c.look_direction.x ) + _ + this.approx( c.look_direction.y ) + _ + this.approx( c.look_direction.z ) + _ + 
				...
				this.approx( c.act_x ) + _ + this.approx( c.act_y ) + _ + 
				this.approx( c.sit ) + _ + 
				this.approx( c.walk_phase ) + _ + 
				this.approx( c.hook_enabled ? 1 : 0 ) + _ + 
				this.approx( c.hook_pos.x ) + _ + this.approx( c.hook_pos.y ) + _ + this.approx( c.hook_pos.z );}
		else
		...
		if ( command === sdSync.COMMAND_I_SPAWN_BULLET ){
			...
			message = 
				command + _ + 
				this.approx( b.x ) + _ + this.approx( b.y ) + _ + this.approx( b.z ) + _ + 
				//this.approx( 0 ) + _ + 
				//this.approx( 0 ) + _ + 
				//this.approx( 0 ) + _ + 
				this.approx( b.tox ) + _ + this.approx( b.toy ) + _ + this.approx( b.toz ) + _ + 
				this.approx( b.dx ) + _ + this.approx( b.dy ) + _ + this.approx( b.dz ) + _ + 
				this.approx( args[ 1 ] ) + _ +
				this.approx( b.local_peer_uid ) + _ +
				this.approx( args[ 2 ] );
		
			self_sent_in = 60;}
		else if ( command === sdSync.COMMAND_I_MOVE_BULLET ){
			let b = args[ 0 ];
			message = 
				command + _ + 
				this.approx( b.local_peer_uid ) + _ + 
				this.approx( b.x ) + _ + this.approx( b.y ) + _ + this.approx( b.z ) + _ + 
				this.approx( b.tox ) + _ + this.approx( b.toy ) + _ + this.approx( b.toz );}
		else if ( command === sdSync.COMMAND_I_DAMAGE_PUSH_PLAYER ){
			let c = args[ 0 ];			
			let dmg = args[ 1 ];
			let tox = args[ 2 ];let toy = args[ 3 ];let toz = args[ 4 ];
			let hit_x = args[ 5 ];let hit_y = args[ 6 ];let hit_z = args[ 7 ];
			...
			message = 
				...
				this.approx( c.uid ) + _ + 
				this.approx( dmg ) + _ + 
				this.approx( tox ) + _ + this.approx( toy ) + _ + this.approx( toz ) + _ + 
				this.approx( hit_x ) + _ + this.approx( hit_y ) + _ + this.approx( hit_z );
		
			//self_sent_in = 60;
			self_sent_in = 120;}
		else if ( command === sdSync.COMMAND_I_SPAWN_GORE || 
			...
			for ( var i = 0; i < args.length; i++ ) message += _ + this.approx( args[ i ] );
		
			self_sent_in = 120; }
		else if ( command === sdSync.COMMAND_I_SAY ) {
			message = command;
		
			for ( var i = 0; i < args.length; i++ ) message += _ + args[ i ];
		
			self_sent_in = 60; }
		else if ( command === sdSync.COMMAND_I_REMOVE_BULLET ) {
			let b = args[ 0 ];
			message = command + _ + this.approx( b.local_peer_uid );
		
			self_sent_in = 60;	}
		else if ( command === sdSync.COMMAND_I_DRAW_EXPLOSION ) {
			let b = args[ 0 ];
			message = 
				command + _ + 
				this.approx( b.x ) + _ + this.approx( b.y ) + _ + this.approx( b.z ) + _ + 
	...
	DecodeCommand( m ) {
		if ( m === undefined ){console.log('Corrupted command? Ignoring for now: '+m);return false;}
		...
		if ( command === sdSync.COMMAND_CHARACTER_STATE ) {
			...
			let i = 1;
			c.x = Number( parts[ i++ ] );c.y = Number( parts[ i++ ] );c.z = Number( parts[ i++ ] );
			c.tox = Number( parts[ i++ ] );c.toy = Number( parts[ i++ ] );c.toz = Number( parts[ i++ ] );
			c.look_direction.x = Number( parts[ i++ ] );c.look_direction.y = Number( parts[ i++ ] );c.look_direction.z = Number( parts[ i++ ] );
			c.walk_vector_xz.x = Number( parts[ i++ ] );c.walk_vector_xz.y = Number( parts[ i++ ] );
			...
			c.act_x = Number( parts[ i++ ] );c.act_y = Number( parts[ i++ ] );
			c.sit = Number( parts[ i++ ] );
			c.walk_phase = Number( parts[ i++ ] );
			var hook_enabled = ( Number( parts[ i++ ] ) === 1 );
			var hook_pos_x = Number( parts[ i++ ] );var hook_pos_y = Number( parts[ i++ ] );var hook_pos_z = Number( parts[ i++ ] );
			
			if ( c.hook_enabled ){if ( !hook_enabled ) c.hook_enabled = false;}
			if ( hook_enabled ) c.HookHere( hook_pos_x, hook_pos_y, hook_pos_z );
		...
		if ( command === sdSync.COMMAND_I_SPAWN_BULLET ) {
			...
			let bullet = sdBullet.CreateBullet({ 
				x: Number( parts[ i++ ] ), y: Number( parts[ i++ ] ), z: Number( parts[ i++ ] ),
				visual_x: visual.x,visual_y: visual.y,visual_z: visual.z,
				tox: Number( parts[ i++ ] ),toy: Number( parts[ i++ ] ),toz: Number( parts[ i++ ] ),
				dx: Number( parts[ i++ ] ),dy: Number( parts[ i++ ] ),dz: Number( parts[ i++ ] ),
				owner: c,
			...
			for ( let i = 0; i < sdBullet.bullets.length; i++ ){
				...
				if ( b.local_peer_uid === local_peer_uid ){
					b.x = Number( parts[ 2 ] );b.y = Number( parts[ 3 ] );b.z = Number( parts[ 4 ] );
					b.tox = Number( parts[ 5 ] );b.toy = Number( parts[ 6 ] );b.toz = Number( parts[ 7 ] );
					break;}}
			return true;	}
		else if ( command === sdSync.COMMAND_I_DAMAGE_PUSH_PLAYER ){
			...
			for ( let i = 0; i < sdCharacter.characters.length; i++ ){
				...
				if ( c.uid === uid ){
					...
					c.tox += Number( parts[ 3 ] );c.toy += Number( parts[ 4 ] );	c.toz += Number( parts[ 5 ] );
					break;}	}
			return true;}
		else if ( command === sdSync.COMMAND_I_SPAWN_GORE ){
			...
			return true;}
		else if ( command === sdSync.COMMAND_I_BULLET_HIT_WORLD ){
			let tx = Number( parts[ 1 ] );let ty = Number( parts[ 2 ] );let tz = Number( parts[ 3 ] );
			...
			if ( is_sniper ) main.WorldPaintDamage( tx, ty, tz, 4.5 );
			...
			return true; }
		else if ( command === sdSync.COMMAND_I_BUILD ) {
			let new_x = Number( parts[ 1 ] );let new_y = Number( parts[ 2 ] );let new_z = Number( parts[ 3 ] );
			let rad = Number( parts[ 4 ] );
			let r = Number( parts[ 5 ] );let g = Number( parts[ 6 ] );let b = Number( parts[ 7 ] );
			...
			for ( let i = 0; i < sdBullet.bullets.length; i++ ){
				...
				if ( b.owner === c )
				if ( b.local_peer_uid === local_peer_uid ){
					b.x = Number( parts[ 1 ] );b.y = Number( parts[ 2 ] );b.z = Number( parts[ 3 ] );
			...
			if ( found )return true;	}
		else if ( command === sdSync.COMMAND_I_DIRECT_HIT_ATOM ) {
			let c = this.parent_dataConnection.character;
			let best_hit = sdAtom.atoms[ Number( parts[ 1 ] ) ];
			let tox = Number( parts[ 2 ] );let toy = Number( parts[ 3 ] );let toz = Number( parts[ 4 ] );
			let local_peer_uid = Number( parts[ 5 ] );
			
			let hp_damage = Number( parts[ 6 ] );let hp_damage_head = Number( parts[ 7 ] );
			...
			if ( bullet !== null ){
				...
				return true;	}}
		else if ( command === sdSync.COMMAND_I_RESSURECT ){let c = this.parent_dataConnection.character;c.Ressurect( false );return true;}
		else if ( command === sdSync.COMMAND_I_RELOAD ) {let c = this.parent_dataConnection.character;c.ReloadIfPossible();return true;}
		else if ( command === sdSync.COMMAND_I_SAY ){
			main.onChatMessage( 'Player #'+this.parent_dataConnection.user_uid, parts[ 1 ].split('<').join('&lt;').split('>').join('&gt;'), this.parent_dataConnection.character );return true;}
...
class ReceivedID{constructor( id ){this.id = id;this.timeout_in = 90;}}
