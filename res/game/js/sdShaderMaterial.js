...
class sdShaderMaterial{
	static init_class(){
		...
		sdShaderMaterial.position_formula = `gl_Position = projectionMatrix * modelViewMatrix * vec4(position,1.0);`;}
	static GenerateDynamicLightUniformsCode(){
		var s = '';
		for ( var i = 0; i < sdShaderMaterial.max_lamps; i++ ){s += 'uniform vec3 lamp'+i+'_pos;\n';s += 'uniform vec3 lamp'+i+'_color;\n';}
		return s;}
	...
	static GetDynamicBrithnessShaderless( x,y,z, target_uniform ){
		...
		for ( var m = 0; m < tot; m++ ){
			...
			if ( di < sdShaderMaterial.lamp_range * sdShaderMaterial.lamp_range ){
				...
				if ( light_intens > 0.8 )light_intens = 0.8;
				
				//light_intens *= 8;
				light_intens *= 4;
				
				var c = main.materials_with_dyn_light[ 0 ].uniforms[ 'lamp' + m + '_color' ].value;
				r += light_intens * c.r;g += light_intens * c.g;b += light_intens * c.b;}}
		
		target_uniform.brightness_r.value = r;target_uniform.brightness_g.value = g;target_uniform.brightness_b.value = b;}
	...
	static CreateMaterial( texture, method ) // particle 
	{
		...
		if ( method === 'explosion' ){
			mat = new THREE.ShaderMaterial({
				uniforms: {
					brightness_r: { type: "f", value: 1 },brightness_g: { type: "f", value: 1 },brightness_b: { type: "f", value: 1 },
					r: { type: "f", value: 1 },g: { type: "f", value: 1 },b: { type: "f", value: 0 },
					opacity: { type: "f", value: 1 }},
				depthWrite: false,transparent: true,flatShading: true,
				extensions: {
				...
				vertexShader: `
				varying vec4 pos;varying vec3 vert_pos;
				void main() {
					${sdShaderMaterial.position_formula}
					pos = gl_Position;vert_pos = position;}
				`,
				fragmentShader: `
				varying vec4 pos;varying vec3 vert_pos;
				uniform float r;uniform float g;uniform float b;uniform float opacity;
				
				void main(){
					//gl_FragColor.rgba = vec4( 1.0, 1.0, 1.0, 0.9 );
					gl_FragColor.rgba = vec4( r * 2.0, g * 2.0, b * 2.0, 0.7 * 2.0 * opacity );
				
					if ( vert_pos.z < 0.3 ) gl_FragColor.rgba = vec4( r, g, b, 0.7 * opacity );
		...
		if ( method === 'sprite' ){
			mat = new THREE.ShaderMaterial({
				uniforms: {
					...
					fog: { type: "c", value: new THREE.Color( 0x000000 ) },fog_intensity: { type: "f", value: 0 },
					brightness_r: { type: "f", value: 999 },brightness_g: { type: "f", value: 999 },brightness_b: { type: "f", value: 999 },
					diffuse: { type: "c", value: new THREE.Color( 0xffffff ) },
					depth_offset: { type: "f", value: 1.5 },
					opacity: { type: "f", value: 1 }},
				depthWrite: false,transparent: true,flatShading: true,
				extensions: {derivatives: false,fragDepth: true, drawBuffers: false,shaderTextureLOD: false},
				vertexShader: `
				varying vec2 vUv;varying vec4 pos;
				uniform vec3 fog;uniform float fog_intensity;varying float fog_final;
				...
				void main() {
					...
					if ( fog_intensity > 0.0 ){float fog_morph = 1.0 / max( 1.0 + gl_Position.z * 0.017, 1.0 );fog_final = fog_morph;}
					else{fog_final = 1.0;}}
				`,
				fragmentShader: `
				uniform sampler2D tDiffuse;varying vec2 vUv;
				varying vec4 pos;
				varying float fog_final;uniform vec3 fog;
				uniform float brightness_r;uniform float brightness_g;uniform float brightness_b;
				uniform vec3 diffuse;uniform float depth_offset;uniform float opacity;
				...
				void main(){
					...
					gl_FragColor.r /= gl_FragColor.a;gl_FragColor.g /= gl_FragColor.a;gl_FragColor.b /= gl_FragColor.a;
				
					gl_FragColor.rgb *= diffuse.rgb;gl_FragColor.a *= opacity;
		...
		if ( method === 'particle' ){
			mat = new THREE.ShaderMaterial({
				uniforms: {
					...
					lamp0_pos: { type: "v3", value: new THREE.Vector3() },lamp0_color: { type: "c", value: new THREE.Color( 0x000000 ) },
					lamp1_pos: { type: "v3", value: new THREE.Vector3() },lamp1_color: { type: "c", value: new THREE.Color( 0x000000 ) },
					lamp2_pos: { type: "v3", value: new THREE.Vector3() },lamp2_color: { type: "c", value: new THREE.Color( 0x000000 ) },
					lamp3_pos: { type: "v3", value: new THREE.Vector3() },lamp3_color: { type: "c", value: new THREE.Color( 0x000000 ) },
					lamp4_pos: { type: "v3", value: new THREE.Vector3() },lamp4_color: { type: "c", value: new THREE.Color( 0x000000 ) },
					lamp5_pos: { type: "v3", value: new THREE.Vector3() },lamp5_color: { type: "c", value: new THREE.Color( 0x000000 ) },
					lamp6_pos: { type: "v3", value: new THREE.Vector3() },lamp6_color: { type: "c", value: new THREE.Color( 0x000000 ) },
					lamp7_pos: { type: "v3", value: new THREE.Vector3() },lamp7_color: { type: "c", value: new THREE.Color( 0x000000 ) }
				},
				depthWrite: true,depthTest: true,transparent: false,flatShading: true,
				extensions: {derivatives: false,fragDepth: true,drawBuffers: false,shaderTextureLOD: false},
				vertexShader: `
				uniform float screen_height;uniform vec3 fog;uniform float dot_scale;
				attribute vec4 colo;
				varying vec4 rgba;varying vec4 rgba2;
				attribute float uv2;
				...
				void main(){
					...
					if ( is_visible ){
						...
						pre_calc.x = gl_Position.z;pre_calc.y = gl_Position.w;pre_calc.z = dot_scale * colo.a / gl_Position.z / gl_Position.z * 0.5;}
					else{rgba.a = 0.0;gl_PointSize = 0.0;gl_Position.xy = vec2( -1.0, -1.0 );}}`,
				fragmentShader: `
				varying vec4 rgba; varying vec4 rgba2;
				varying vec3 pre_calc;
			...
			mat.ScreenUpdated = function(){mat.uniforms.screen_height.value = main.DisplayedScreen.height * 75 / main.main_camera.fov * main.pixel_ratio;};}
		else
		if ( method === 'color' ){
			mat = new THREE.ShaderMaterial({
				uniforms: {diffuse: { type: "c", value: new THREE.Color( 0xffffff ) },opacity: { value: 1 },depth_offset: { type: "f", value: 1.5 }},
				depthWrite: true,transparent: false,flatShading: true,
				extensions: {derivatives: false,fragDepth: true, drawBuffers: false,shaderTextureLOD: false},
				vertexShader: `
		...
		if ( method === 'voxel_map' ) {
			mat = new THREE.ShaderMaterial({
				...
				extensions: {derivatives: false,fragDepth: true, drawBuffers: false,shaderTextureLOD: false},
				vertexShader: `
				varying vec2 vUv; varying vec4 pos; varying vec4 world_pos;
				uniform vec3 fog; uniform float fog_intensity; varying float fog_final;
				...
				varying vec4 pos;varying vec4 world_pos;
		...
		else throw new Error('sdShaderMaterial: '+method+' is not a defined method');
		
		Object.defineProperty( mat, "color",{get: function color() { return this.uniforms.diffuse.value; },set: function color( v ) { this.uniforms.diffuse.value = v; }});
		Object.defineProperty( mat, "opacity",{get: function opacity() { return this.uniforms.opacity.value; },set: function opacity( v ) { this.uniforms.opacity.value = v; }});
		if ( mat.uniforms.tDiffuse !== undefined ){
			Object.defineProperty( mat, "map", {get: function map() { return this.uniforms.tDiffuse.value; },set: function map( v ) { this.uniforms.tDiffuse.value = v; }});}
		return mat;}
	
	static getStackTrace(){var obj = {};Error.captureStackTrace( obj, sdShaderMaterial.getStackTrace );return obj.stack;}}
