...
class sdChain{
	...
	static CreateChain( a, b, def, in_limb ){
		if ( sdChain.enable_reuse ){
			...
			if ( sdChain.chains.length < min_i ) throw new Error('Where all the starter chains gone? They should exist at this moment.');
...
class sdAtom{
	static init_class(){
		sdAtom.atoms = [];sdAtom.pseudo_atoms = []; // Copies that stay for some time
		
		sdAtom.snow_particles = null;sdAtom.snow_particles_last_trace = null;sdAtom.snow_particles_tested_i = 0;sdAtom.snow_particles_xyz_addition = 100;
		...
		mini_texture.src = "assets/voxel_player3.png";mini_texture.onload = function() {bmp.ctx.drawImage( mini_texture, 0, 0 );};
	...
	static RandomizeStars(){
		...
		for ( var i = 0; i < 200; i++ ){
			...
			sdAtom.stars.push( v );}}
	static init(){
		sdAtom.material.uniforms.fog.value = new THREE.Color( main.fog_color );sdAtom.material.uniforms.dot_scale.value = sdAtom.atom_scale;
		...
		if ( sdAtom.mesh === null )
		{
			...
			for ( var i = 3; i < rgba.length; i += 4 ) rgba[ i ] = 0;

			sub_geom.updateVertexDataTyped( vertices );sub_geom.updateRGBADataTyped( rgba );sub_geom.updateSecondaryUVDataTyped( uvs2 );

			sdAtom.sub_geom = sub_geom;sdAtom.vertices = vertices;sdAtom.rgba = rgba;sdAtom.uvs2 = uvs2;
		...
		if ( main.isWinter ){
			sdAtom.snow_particles = [];sdAtom.snow_particles_last_trace = [];
			for ( var i = 0; i < main.level_chunks_x * main.level_chunks_z * main.chunk_size * main.chunk_size * 0.1; i++ )
			{
				...
				sdAtom.snow_particles.push( v );sdAtom.snow_particles_last_trace.push( v2 );}}}
	...
	constructor( x, y, z, r, g, b, material, model_x, model_y, model_z, parent, glowing ){
		...
		this.x = this.lx = x;this.y = this.ly = y;this.z = this.lz = z;
		
		// Where it will show up on model 
		this.model_x = model_x;this.model_y = model_y;this.model_z = model_z;
		...
		this.r_initial = r;this.g_initial = g;this.b_initial = b;
		...
		this.material = material;this.material_initial = material;
		
		this.temp_grav_disable_tim = 0;
		
		this.my_chains = [];this.my_chains_initial_length = 0;
	...
	remove(){
		if ( !this.removed ){ this.removed = true;
			for ( var i = 0; i < this.my_chains.length; i++ ) this.my_chains[ i ].remove();}}
	...
	AddSpeedPosFrictionFractal( bx, by, bz, x, y, z, fx, fy, fz, GSPEED ) // Only for collisions with world
	{
		for ( var i = 0; i < this.my_chains.length; i++ ){
			...
			if ( !c.in_limb ) continue;

			var other = ( c.a === this ) ? c.b : c.a;

			if ( bx !== 0 ) other.tox = Math.abs( other.tox ) * bx;
			if ( by !== 0 ) other.toy = Math.abs( other.toy ) * by;
			if ( bz !== 0 ) other.toz = Math.abs( other.toz ) * bz;

			other.x += x;other.y += y;other.z += z;
			other.tox = main.MorphWithTimeScale( other.tox, 0, fx, GSPEED );other.toy = main.MorphWithTimeScale( other.toy, 0, fy, GSPEED );other.toz = main.MorphWithTimeScale( other.toz, 0, fz, GSPEED );
			
			if ( by > 0 ) other.temp_grav_disable_tim = 0;}
		
		if ( bx !== 0 )this.tox = Math.abs( this.tox ) * by; if ( by !== 0 )this.toy = Math.abs( this.toy ) * by; if ( bz !== 0 )this.toz = Math.abs( this.toz ) * by;
			
		this.x += x;this.y += y;this.z += z;

		this.tox = main.MorphWithTimeScale( this.tox, 0, fx, GSPEED );this.toy = main.MorphWithTimeScale( this.toy, 0, fy, GSPEED );this.toz = main.MorphWithTimeScale( this.toz, 0, fz, GSPEED );
			
		if ( by > 0 )this.temp_grav_disable_tim = 0;}
	
	WakeUp(){this.sleep_tim = 0;}
	...
	static ThinkNow( GSPEED ){
		...
		for ( var step = 0; step < steps; step++ ){
					...
					if ( a.material > sdAtom_MATERIAL_ALIVE_PLAYER_GUN ){
						if ( !ch.removed ){
							...
							if ( a.sleep_tim < 1 || b.sleep_tim < 1 ){
								...
								if ( di > 0 ) {dx /= di;dy /= di;dz /= di;

									if ( ch.in_limb )
									//if ( di > target_di + 2 * GSPEED )
									//if ( di > target_di + 2 + 2 * GSPEED )
									if ( di > target_di + 8 ){
										sdBullet.DrawSingleAtomDamage( ch.a );sdBullet.DrawSingleAtomDamage( ch.b );
										ch.remove();
								...
								if ( di > 0.1 ){
									var cx = ( a.x + b.x ) / 2;var cy = ( a.y + b.y ) / 2;var cz = ( a.z + b.z ) / 2;
									var power = 0.25;var power_pos = 0.5;
								/*
									var xx = ( cx - dx * target_di * 0.5 - a.x );
									var yy = ( cy - dy * target_di * 0.5 - a.y );
									var zz = ( cz - dz * target_di * 0.5 - a.z );
									*/
									var xx = ( cx - a.x ) / di * ( di - target_di );var yy = ( cy - a.y ) / di * ( di - target_di );var zz = ( cz - a.z ) / di * ( di - target_di );

									a.tox += xx * power;a.toy += yy * power;a.toz += zz * power;
									b.tox -= xx * power;b.toy -= yy * power;b.toz -= zz * power;

									a.x += xx * power_pos;a.y += yy * power_pos;a.z += zz * power_pos;
									b.x -= xx * power_pos;b.y -= yy * power_pos;b.z -= zz * power_pos;}}}}
			...
			for ( var i = 0; i < sdAtom.atoms.length; i++ ){
				...
				if ( step === 0 ) {a.lx = a.x;a.ly = a.y;a.lz = a.z;}
						...
						if ( c.x < 0 || c.z < 0 || c.x > main.level_chunks_x * main.chunk_size || c.z > main.level_chunks_z * main.chunk_size ){
							...
							if ( c.y < di_out ){
								...
								if ( di > 0.01 )
								{
									...
									c.tox = main.MorphWithTimeScale( c.tox, 0, 0.7, GSPEED );c.toy = main.MorphWithTimeScale( c.toy, 0, 0.7, GSPEED );c.toz = main.MorphWithTimeScale( c.toz, 0, 0.7, GSPEED );
									c.toy += 1 * GSPEED;c.tox += ( 1 ) * dx * GSPEED;c.toz += ( 1 ) * dz * GSPEED;}}}}

					if ( a.sleep_tim < 1 ){
						if ( Math.pow( a.tox, 2 ) + Math.pow( a.toy, 2 ) + Math.pow( a.toz, 2 ) < 0.25 ){a.sleep_tim += GSPEED * 0.01;}
						else{a.sleep_tim = 0;}

						var tx = a.x + a.tox * GSPEED;var ty = a.y + a.toy * GSPEED;var tz = a.z + a.toz * GSPEED;
						var y_offset = 1;

						var morph = main.TraceLine( a.x, a.y - y_offset, a.z, tx, ty - y_offset, tz, null, 1, 0 );

						if ( morph === 1 ){a.x = tx;a.y = ty;a.z = tz;
							if ( a.temp_grav_disable_tim < 1 ) a.temp_grav_disable_tim = Math.min( 1, a.temp_grav_disable_tim + GSPEED * 0.15 );
							if ( a.temp_grav_disable_tim >= 0 ) a.toy -= GSPEED * 0.1 * a.temp_grav_disable_tim;}
						else{
							var translate = 1;
							...
							if ( main.TraceLine( a.x, a.y + y_offset + translate, a.z,  a.x, a.y + y_offset + translate, a.z, null, 1, 0 ) === 1 ){
								a.toy = Math.abs( a.toy * 0.2 );a.tox *= 0.9;a.toz *= 0.9;		
								a.temp_grav_disable_tim = 0;}
							else if ( main.TraceLine( a.x + y_offset + translate, a.y, a.z, a.x + y_offset + translate, a.y, a.z, null, 1, 0 ) === 1 ){
								a.tox = Math.abs( a.tox * 0.2 );a.toy *= 0.9;a.toz *= 0.9;}
							else if ( main.TraceLine( a.x - y_offset - translate, a.y, a.z,  a.x - y_offset - translate, a.y, a.z, null, 1, 0 ) === 1 ){
								a.tox = -Math.abs( a.tox * 0.5 );a.toy *= 0.9;a.toz *= 0.9;}
							else if ( main.TraceLine( a.x, a.y, a.z + y_offset + translate, a.x, a.y, a.z + y_offset + translate, null, 1, 0 ) === 1 ){
								a.toz = Math.abs( a.toz * 0.2 ); a.toy *= 0.9; a.tox *= 0.9;}
							else if ( main.TraceLine( a.x, a.y, a.z - y_offset - translate,  a.x, a.y, a.z - y_offset - translate, null, 1, 0 ) === 1 ){
								a.toz = -Math.abs( a.toz * 0.2 ); a.toy *= 0.9; a.tox *= 0.9;}
							else{
								/*a.x -= a.tox;
				...
				{
					vertices[ point * 3     ] = a.x;vertices[ point * 3 + 1 ] = a.y;vertices[ point * 3 + 2 ] = a.z;

					rgba[ point * 4     ] = a.r;rgba[ point * 4 + 1 ] = a.g;rgba[ point * 4 + 2 ] = a.b;rgba[ point * 4 + 3 ] = 1;

					if ( a.glowing > 0 ) uvs2[ point ] = main.GetEntityBrightness( a.x, a.y, a.z ) + a.glowing;
					else if ( a.glowing < 0 ) uvs2[ point ] = main.GetEntityBrightness( a.x, a.y, a.z ) / ( 1 - a.glowing );
					else uvs2[ point ] = main.GetEntityBrightness( a.x, a.y, a.z );
		...
		for ( var i = 0; i < sdAtom.pseudo_atoms.length; i++ ){
			...
			if ( a.ttl <= 0 ){
				...
				if ( sdAtom.pseudo_atoms[ i + more ].ttl > 0 ) break;
			...
			rgba[ point * 4     ] = a.r;rgba[ point * 4 + 1 ] = a.g;rgba[ point * 4 + 2 ] = a.b;rgba[ point * 4 + 3 ] = a.ttl / sdAtom.pseudo_atoms_ttl_max;
			...
			a.x += a.tox * GSPEED;a.y += a.toy * GSPEED;a.z += a.toz * GSPEED;
			
			a.tox = main.MorphWithTimeScale( a.tox, 0, 0.9, GSPEED );a.toy = main.MorphWithTimeScale( a.toy, 0, 0.9, GSPEED );a.toz = main.MorphWithTimeScale( a.toz, 0, 0.9, GSPEED );
		...
		for ( var i = 0; i < sdCharacter.characters.length; i++ ){
			...
			if ( sdCharacter.characters[ i ].hook_enabled ){
				...
				for ( var i2 = 0; i2 < di; i2++ ){
					...
					rgba[ point * 4     ] = ( i2 % 4 < 2 ) ? 0.2 : 0.1;rgba[ point * 4 + 1 ] = ( i2 % 4 < 2 ) ? 0.2 : 0.1;rgba[ point * 4 + 2 ] = ( i2 % 4 < 2 ) ? 0.2 : 0.1;
			...
			if ( c.cur_weapon_object.gun_class.has_muzzle_flash ){
				...
				for ( var f = 0; f < 16; f++ ){
					...
					rgba[ point * 4     ] = c.muzzle_r;rgba[ point * 4 + 1 ] = c.muzzle_g;rgba[ point * 4 + 2 ] = c.muzzle_b;
		...
		for ( var i = 0; i < sdBullet.bullets.length; i++ ){
			...
			if ( b.is_melee ) continue;
			
			if ( b.is_sniper ) continue;
			...
			if ( is_rocket || is_plasma ) main.DrawDynamicLight( b.x, b.y, b.z, b.r, b.g, b.b );
			else main.DrawDynamicLight( b.x, b.y, b.z, b.r * 0.2, b.g * 0.2, b.b * 0.2 );
		
			if ( is_plasma ){
				dx = b.tox;dy = b.toy;dz = b.toz;
			...
			{dx = b.tox - main.speed.x;dy = b.toy - main.speed.y;dz = b.toz - main.speed.z;
				length = 10; // 6
			}

			var di = Math.sqrt( main.Dist3D_Vector_pow2( dx, dy, dz ) ) / length;
			if ( di > 0.001 ){dx /= di;dy /= di;dz /= di;}
			...
			for ( var i2 = 0; i2 <= length; i2 += sdAtom.atom_scale ){
				var morph = i2 / length;
				vertices[ point * 3     ] = bx + dx * morph;vertices[ point * 3 + 1 ] = by + dy * morph;vertices[ point * 3 + 2 ] = bz + dz * morph;

				rgba[ point * 4 + 3 ] = 1; // Scale
				if ( is_plasma ){
					rgba[ point * 4     ] = b.r * 2;rgba[ point * 4 + 1 ] = b.g * 2;rgba[ point * 4 + 2 ] = b.b * 2;
					uvs2[ point ] = 1;
				}
				else if ( is_rocket ){
					if ( i2 < 1 ){
						rgba[ point * 4     ] = 255 / 255 * 1.5;rgba[ point * 4 + 1 ] = 251 / 255 * 1.5;rgba[ point * 4 + 2 ] = 192 / 255 * 1.5;
						uvs2[ point ] = 1;
						
						if ( i2 === 0 )rgba[ point * 4 + 3 ] = 0.5; // Scale
						if ( i2 === 0.5 )rgba[ point * 4 + 3 ] = 0.7; // Scale
					}
					else
					if ( i2 === 1 ){
						rgba[ point * 4     ] = 255 / 255 * 1.5;rgba[ point * 4 + 1 ] = 251 / 255 * 1.5;rgba[ point * 4 + 2 ] = 192 / 255 * 1.5;
						...
						rgba[ point * 4     ] = 0.3;rgba[ point * 4 + 1 ] = 0.3;rgba[ point * 4 + 2 ] = 0.3;
						uvs2[ point ] = main.GetEntityBrightness( vertices[ point * 3     ], vertices[ point * 3 + 1 ], vertices[ point * 3 + 2 ] );
						rgba[ point * 4 + 3 ] = 1 - Math.pow( i2 / length, 6 ); // Scale
					}}
				else{
					rgba[ point * 4     ] = b.r;rgba[ point * 4 + 1 ] = b.g;rgba[ point * 4 + 2 ] = b.b;
					uvs2[ point ] = 1;
		...
		if ( main.my_character.cur_weapon_object.gun_class.is_build_tool )
		{
			...
			new_x -= c.x;new_y -= c.y;new_z -= c.z;

			var di = main.Dist3D( 0, 0, 0, new_x, new_y, new_z );

			new_x /= di / ( rad + sdCharacter.player_half_height + 1 );new_y /= di / ( rad + sdCharacter.player_half_height + 1 );new_z /= di / ( rad + sdCharacter.player_half_height + 1 );
			new_x += c.x;new_y += c.y;new_z += c.z;						
			new_x = Math.round( new_x );new_y = Math.round( new_y );new_z = Math.round( new_z );
			...
			for ( var z = -rad+1; z < rad; z++ ){
				...
				if ( di > rad - 1 ){
					...
					if ( br > 0.25 ){
						vertices[ point * 3     ] = new_x + x + 0.5;vertices[ point * 3 + 1 ] = new_y + y + 0.5;vertices[ point * 3 + 2 ] = new_z + z + 0.5;
		...
		var point2 = point;
		while ( point2 < sdAtom.last_point ){uvs2[ point2++ ] = -1;}
...
class PseudoAtom{
	constructor( atom ){
		this.x = atom.x;this.y = atom.y;this.z = atom.z;
		this.tox = atom.tox;this.toy = atom.toy;this.toz = atom.toz;
		this.r = atom.r;this.g = atom.g;this.b = atom.b;
