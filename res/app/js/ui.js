...
function showList(area) {  $("#registry").html(''); $("#registry").hide(); 
    switch (area) {
        case "domain": $("#registry-domain").html(''); openDomain(); sortSet = [area,domains,0,18,16]; for (let i=0;i<domains.length;i++) { if (domains[i].hide == false) addList(i); } 
		    $("#registry-domain").css("height",(height-16) + "px"); //adjustSort(true); 
		    break;
		...
		if (opens[i].core.slug != "ocur" && opens[i].core.slug != "nfnth") { addOpen(i); } 
		else { if (opens[i].core.slug == "ocur" && nfnth == true) { addOpen(i); } 
			else if (opens[i].core.slug == "urland" && boxer.special == true) { addOpen(i); } 
			else { opens[i].checked = false; } } } //adjustSort(false);  
		    $("#registry-open").css("height",(height-16) + "px"); showList('domain'); break; }
...
function addListDetail(i) { 
	...
	for (var a = 0; a < opens.length; a++) { if (opens[a].core.slug == domains[i].core.collection.slug) card = opens[a].core.banner_image_url; }
     	$("#domain-info").html('<div style="display:flex;justify-content:space-evenly;align-items:center;width:100%;padding-top:12px;"><div style="display: flex;flex-direction: column;text-align: left;align-items:center;width:100%;margin-top:12px;"><div style="display:flex; justify-content:center; width:80%;flex-direction:column;"><div style="cursor:pointer;align-items:center;display: flex;justify-content: center;">' + makeCardOwner(i) + '</div><div style="margin-left:24px; display:flex; flex-direction:column;justify-content:space-evenly;"><a id="trigger" class="quickx hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="searcher(' + i + ');">✉️</a><a class="quickx hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="setDialog(renderMd(\'' + domains[i].owner.content + '\'));">📄</a></div></div><div style="margin-top:24px; display:flex;justify-content:center;">' + domains[i].owner.name + '&nbsp;&nbsp · &nbsp;&nbsp<a onclick="openInNewTab(\'' + domains[i].core.external_link + '\');">' + domains[i].owner.icon + '</a></div><br/></div></div>');
$('#modal1').modal('open');
...
//function change(a){ document.getElementById('vid').poster=a; document.getElementById('vid').src=a.replace('res/raw/','res/media/').replace('.jpg','.mp4'); }
		...
		$.getJSON("https://en.wikipedia.org/w/api.php?origin=*&format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&titles=" + search, function (data) { var fuller = "No data found.";
			$.each(data.query.pages, function(key, value) { fuller = value.extract; });
			setDialog("<div style='text-align:center;display:flex;justify-content:center;'><p style='display:flex;justify-content:center;align-items:center;'>" + domains[i].name + "&nbsp;&nbsp; · &nbsp;&nbsp;<a style='font-weight:16px;' onclick='openInNewTab(\"" + domains[i].core.external_link + "\");'>" + domains[i].core.name + "</a></p></div><div style='text-align:center;display:flex;justify-content:center;'><p style='display:flex;justify-content:center;align-items:center;'><img style='display:flex; cursor:pointer;' class='z-depth-1' width='30' height='30' src='res/img/key/" + getCollect(domains[i].core.collection.slug) + "'/>&nbsp;&nbsp;&nbsp;&nbsp;<a style='font-weight:bold;color:" + getFront(domains[i].core.collection.name) + "'>" + domains[i].core.collection.name + "</a></p></div><br/><div id='emDisp' style='text-align:center;'>" + embedDisplay + "</div></br><p>" + fuller + "</p><br/><a onclick='openInNewTab(\"" + domains[i].ref + "\");'>Read more...</a><br/><br/><p><img style='max-width:80%;max-height:600px;object-fit:cover;' src='" + domains[i].res + "' /></p>");  });
			 
			 //return "<div style='text-align:center;display:flex;justify-content:center;'><p style='display:flex;justify-content:center;align-items:center;'><a style='font-weight:16px;' onclick='openInNewTab(\"" + domains[i].core.external_link + "\");'>" + domains[i].core.name + "</a>&nbsp;&nbsp; · &nbsp;&nbsp;<img style='display:flex; cursor:pointer;' class='z-depth-1' width='30' height='30' src='res/img/key/" + getCollect(domains[i].core.collection.slug) + "'/><a>&nbsp;&nbsp;" + domains[i].core.collection.name + "</a></p></div><br/><div id='emDisp' style='text-align:center;'>" + embedDisplay + "</div></br><p>No data found.</p><br/><a>Read more...</a><br/><p><img style='max-width:80%;max-height:600px;object-fit:cover;' src='" + domains[i].res + "' /></p>"; 
	  }
	
function addUserDeed(i) { var embedDisplay = ""; embedDisplay = "<img  style='width:80%;' src='" + domainMd.image + "' />"; // }
...
function addArtDeed(i) { var embedDisplay = ""; embedDisplay = "<img  style='width:80%;' src='" + artifacts[i].image + "' />"; // }
