var opens = [];//, deeds = [], domains = [], artifacts = [], claims = []; 
const options = {method: 'GET'}; 
//var domainFound = false; var tempArtifact;

class Ocur { name = ""; icon = ""; front = ""; back = ""; coord = ""; content = ""; }
class Open { core = "api_object"; count = 0; checked = false; domains = []; } //asset.name; image_url; banner_image_url; slug;
...
class Wallet { address = ""; deed = []; ocur = new Ocur(); } //class Deed { name = ""; id = ""; }

function pullOpens() {
    fetch('https://api.opensea.io/api/v1/collections?asset_owner=0x8a83fbbacb82030ea17179c0403b04e7bce7ba10&offset=0&limit=300', options).then(response => response.json()).then(response => {
        for (let i = 0; i < response.length; i++) { var asset = response[i];
            if (asset.name == "Rarible" || asset.name == "Uniswap V3 Positions") { continue; }
            var tmp = new Open; tmp.core = asset; opens.push(tmp); } } ).then(response => { for (let i=0;i<opens.length;i++) { opens[i].count = pullOpen(opens[i].core.slug, opens[i].domains); 
				if (opens[i].core.slug == "OCUR") { opens[i].core.name = "NfNth"; } }  }).catch(err => console.error(err)); }
function pullOpen(name, domains) { 
    fetch('https://api.opensea.io/api/v1/assets?limit=50&collection='+name, options).then(response => response.json()) //.then(response => assets = response)
      .then(response => { for (let i = 0; i < response.assets.length; i++) {
			...
			if (domainFound == false) { var domainPath = getParameter(); 
				if (domainPath != 'none') { for (let b = 0; b < domains.length; b++) { 
					if (domains[b].core.external_url == domainPath) { showMark(b); document.title = document.title.replace("Ocurious",domains[b].core.external_url); domainFound = true; 
						} } } else { domainFound = true; } }
			complete = domains.slice(); domainFind(); } }).catch(err => console.error(err)); }

function pullArtifact(link) { 
	fetch('https://api.opensea.io/api/v1/asset/'+link.replace('https://opensea.io/assets/',''), options).then(response => response.json())
      		.then(response => {  tempArtifact = response; updatePreview(); }).catch(err => console.error(err)); }

function pullAssets(owner, slug) { 
    fetch('https://api.opensea.io/api/v1/assets?limit=50&owner='+owner.address+'&collection_slug='+slug, options).then(response => response.json())
      .then(response => {
...
function pullOwner(i) { id = domains[i].core.token_id;
	fetch('https://api.opensea.io/api/v1/asset/0x495f947276749ce646f68ac8c248420045cb7b5e/'+id, options).then(response => response.json()) 
...
function pullPrice(i) { id = domains[i].core.token_id;
	fetch('https://api.opensea.io/api/v1/asset/0x495f947276749ce646f68ac8c248420045cb7b5e/'+id+'/listings', options).then(response => response.json()) 
