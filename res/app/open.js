...
function pullAssets(owner, slug) { alert(slug);
    fetch('https://api.opensea.io/api/v1/assets?limit=50&owner='+owner.address+'&collection_slug='+slug, options)
      .then(response => response.json())
      .then(response => {
        for (let i = 0; i < response.assets.length; i++) { owner.deed.push(response.assets[i].token_id); alert(response.assets[i].token_id);
