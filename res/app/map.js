...
function showMark(coord, color, image, link, name, id, area) { clearLearn();
	...
	var style=$('#marker'+id).attr('style'); style += ";align-items: center;justify-content: center;display: flex;background-color: whitesmoke;border: 2px solid darkslategray;font-size: 16px;"; $('#marker'+id).attr('style',style); 
    	//style += ";background-image:url('img/icon/domain/"+folder + "/" +icon+".png'); 
	var markup = '<div><div style="display:flex; justify-content:center; flex-direction:column; align-items:center;" ><div style="width:64px;height:64px;display:flex;margin-top:6px;border-radius:8px;cursor:pointer;" class="z-depth-1" onclick="buildDoc(\'' + id + '\');"><img style="border-radius:8px;width:64px;height:64px;" src="' + domains[id].core.image_url + '" /></div><div style="margin-top:16px; font-size:16px;"><a onclick="openInNewTab(\'' + link + '\');">' + domains[id].core.name + '</a></div></div></div>';
	//var markup = markCode;
...
var editContent = '<div class="editMark" style="width:100%; height:48px;display:flex;justify-content:center;"><a class="quick hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="setShape();">🟢</a><a class="quick hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="switchIcon();">🧸</a><a class="quick hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="colorStart(this);">🎨</a><a class="quick hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="outlineStart(this);">🔳</a></div><div id="header-logo" style="display: flex;justify-content: space-evenly;align-items: center; display: flex;margin-top:8px;"><div style="border-radius:12px; cursor:pointer;display:flex; height:64px; width:64px;" class="z-depth-1"><img width="64" height="64" id="thumb" src="res/img/barrel.png" onclick="$(\'#preview\').trigger(\'click\');" style="border-radius:8px;" /></div></div></div></div><div class="hiddenfile"><input name="upload" type="file" id="preview" onchange="setPreview();" multiple="multiple"/><div style="visibility:hidden;width:0;position:absolute;top:0;"><input type="color" oninput="setColor();" id="favcolor" name="favcolor" value="#ff0000" style="width:0px; height:0px;opacity:0;" /><input type="color" oninput="setOutline();" id="favcolor2" name="favcolor2" value="#ff0000" style="width:0px; height:0px;opacity:0;" /></div></div><div style="padding:12px; padding-top:0px; padding-bottom:0px; display:flex; justify-content:center;"><div style="margin-top:0px;margin-bottom:0px;" class="input-field col s4"><input id="edit-name" type="text" class="validate" onchange="setText();"></div></div><div style="display:flex;justify-content:space-evenly;margin-top:12px;margin-bottom:12px;align-items:center;"><a class="waves-effect waves-light btn-flat" onclick="resetArea();" style="padding:0px;"><i class="material-icons">refresh</i></a><a class="waves-effect waves-green green btn" onclick="showView(\'editor\');">Editor&nbsp;&nbsp;✏️</a></div></div>';
