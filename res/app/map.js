...
function showMark(coord, color, image, link, name, id, area) { 
	...
	var markup = '<div><div style="display:flex; justify-content:center; flex-direction:column; align-items:center;" ><div style="width:64px;height:64px;display:flex;margin-top:6px;border-radius:8px;cursor:pointer;" class="z-depth-1" onclick="buildDoc(\'' + id + '\');"><img style="border-radius:8px;width:64px;height:64px;" src="' + domains[id].core.image_url + '" /></div><div style="margin-top:16px; font-size:16px;padding-right:0px!important;"><a onclick="openInNewTab(\'' + link + '\');">' + name + '</a></div><div class="editMark" style="width:100%; height:48px;display:flex;justify-content:center;"><a class="quickx hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="pullOwner(' + id + ');">📮</a><a class="quickx hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="mapAdd(' + id + ');">📦</a></div></div></div>';
	//var markup = markCode;
...
function showMail() { if (tempMark != "") { tempMark.remove(); tempMark = ""; }
	...
	tempMark.setPopup(new AnimatedPopup({ offset: 25, openingAnimation: {duration: 1000, easing: 'easeOutElastic'}, closingAnimation: { duration: 200, easing: 'easeInBack' } }).setHTML(mailContent));  tempMark.toggle();
