...
function showIntroSim() { clearMap(); document.getElementById('booker').click(); $('#deed-pane').sidenav('close');
	...
	var markup = '<div style="display:flex;flex-direction:column;align-items:center"><div>' + makeCardGame('cadetblue','left') + '</div><div>' + makeCardGame('indianred','right'); + '</div></div><div style="margin-top:16px; font-size:16px;"><span onclick="showDoc(\'tact/a\');" style="cursor:pointer;">Remote Outpost</span><br/><br/><a id="butIntro" class="waves-effect waves-light btn green lighten-2" onclick="popBase.togglePopup(); showIntroArt();"><i class="material-icons right">arrow_forward</i>Next</a></div></div>';
...
function showIntroArt() { startPoint = base; endPoint = learnArt; showPath('indianred'); pather = true; $("#butIntro").addClass("disabled");
