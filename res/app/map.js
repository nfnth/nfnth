...
function showIntroSim() { clearMap(); document.getElementById('booker').click(); $('#deed-pane').sidenav('close');
	...
	var markup = '<div style="display:flex;flex-direction:column;align-items:center">' + makeCard() + '</div><div style="margin-top:16px; font-size:16px;"><span onclick="showDoc(\'tact/a\');" style="cursor:pointer;">Remote Outpost</span><br/><br/><a id="butIntro" class="waves-effect waves-light btn green lighten-2" onclick="popBase.togglePopup(); showIntroArt();"><i class="material-icons right">arrow_forward</i>Next</a></div></div>';
...
function showIntroMap() { clearMap(); document.getElementById('booker').click(); $('#deed-pane').sidenav('close');
	...
	$('#markera').addClass('markre'); $('#markera').addClass('z-depth-3'); $('#markera').addClass('triangle-up'); 
	var style = $('#markera').attr('style'); style += ";background-image:url('res/img/seal3.png');background-size:cover;border:solid 2px darkgrey;border-radius:50%;"; $('#markera').attr('style',style); 
	var markup = '<div style="display:flex;flex-direction:column;align-items:center">' + makeCard() + '</div><div style="margin-top:16px; font-size:16px;"><span onclick="showDoc(\'tact/a\');" style="cursor:pointer;">Use Deed</span><br/><br/><a id="butIntro" class="waves-effect waves-light btn green lighten-2" onclick="popBase.togglePopup(); showIntroArt();"><i class="material-icons right">arrow_forward</i>Next</a></div></div>';
...
function showIntroOwn() { addBeacon(learnOwn); beacon = true; $("#butArt").addClass("disabled");
	...
	var markup = '<div style="display:flex;flex-direction:column;align-items:center">' + makeCard() + '</div><div style="margin-top:16px; font-size:16px;"><span onclick="showDoc(\'tact/c\');" style="cursor:pointer;">Post Note</span><br/><br/><a id="butOwn" class="waves-effect waves-light btn amber lighten-2" onclick="popOwn.togglePopup(); showIntroPath();"><i class="material-icons right">arrow_forward</i>Next</a></div></div>';
...
function showIntroPath() { clearDraw(); startPoint = base; endPoint = learnPath; showPath('goldenrod'); pather = true;  $("#butOwn").addClass("disabled");
	...
	var markup = '<div style="display:flex;flex-direction:column;align-items:center">' + makeCard() + '</div><div style="margin-top:16px; font-size:16px;"><span onclick="showDoc(\'tact/d\');" style="cursor:pointer;">Trade OCUR</span><br/><br/><a id="butPath" class="waves-effect waves-light btn red lighten-2" onclick="showIntroMap();"><i class="material-icons right">restart_alt</i>Reset</a></div></div>';
...
function showMark(coord, color, image, link, name, id, area) { 
	...
	var markup = '<div><div style="display:flex; justify-content:center; flex-direction:column; align-items:center;" ><div style="width:64px;height:64px;display:flex;margin-top:6px;border-radius:8px;cursor:pointer;" class="z-depth-1" onclick="buildDoc(\'' + id + '\');"><img style="border-radius:8px;width:64px;height:64px;" src="' + domains[id].core.image_url + '" /></div><div style="margin-top:16px; font-size:16px;"><a style="padding-right:0px!important;" onclick="openInNewTab(\'' + link + '\');">' + name + '</a></div><div class="editMark" style="width:100%; height:48px;display:flex;justify-content:center;"><a class="quickx hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="pullOwner(' + id + ');">📜</a><a class="quickx hoverable z-depth-1 crisp waves-effect waves-light btn modal-trigger" href="#modal4" onclick="searcher();">📦</a></div></div></div>';
	//var markup = markCode;
...
var mapSpot;
function mapAdd(i) { clearMark(); showView('mapper');
		    
		    for (var a = 0; a < domains.length; a++) { if (domains[a].name = "Majorca") i = a; }
		    mapSpot = converter(domains[i].coord);
...
var editContent = '<div id="header-logo" style="display: flex;justify-content: space-evenly;align-items: center; display: flex;margin-top:8px;margin-bottom:8px;"><div style="border-radius:12px; cursor:pointer;display:flex; height:80px; width:80px;margin-top:8px;" class="z-depth-1"><img width="80" height="80" id="thumb" src="res/img/seal3.png" onclick="$(\'#preview\').trigger(\'click\');" style="border-radius:8px;" /></div><div><a class="waves-effect waves-light btn-flat" style="padding:0px;" onclick="showView(\'editor\');">📝</a></div></div><div class="hiddenfile"><input name="upload" type="file" id="preview" onchange="setPreview();" multiple="multiple"/><div style="visibility:hidden;width:0;position:absolute;top:0;"><input type="color" oninput="setColor();" id="favcolor" name="favcolor" value="#ff0000" style="width:0px; height:0px;opacity:0;" /><input type="color" oninput="setOutline();" id="favcolor2" name="favcolor2" value="#ff0000" style="width:0px; height:0px;opacity:0;" /></div></div><div style="padding:12px; padding-top:0px; padding-bottom:0px; display:flex; justify-content:center;"><div style="margin-top:0px;margin-bottom:0px;" class="input-field col s4"><input id="edit-name" type="text" class="validate" oninput="setText(this);"></div></div><div class="editMark" style="width:100%; height:48px;display:flex;justify-content:center;margin-bottom:24px;"><a id="trigger" class="quickx hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="picMo();">🏳️</a><a class="quickx hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="colorStart(this);">🎨</a><a class="quickx hoverable z-depth-1 crisp waves-effect waves-light btn" onclick="outlineStart(this);">🔳</a></div><div style="display:flex;justify-content:space-evenly;margin-top:16px;margin-bottom:12px;align-items:center;"><a class="waves-effect waves-light btn-flat" onclick="clearEdit();" style="padding:0px;">🗑️</a>&nbsp;&nbsp;&nbsp;&nbsp;<a class="waves-effect waves-green green lighten-1 btn" onclick="alert(\'save\');">💾</a></div></div>';
