...
function showIntroMap() { clearMap(); document.getElementById('booker').click(); $('#deed-pane').sidenav('close');
	...
	$('#markera').addClass('markre'); $('#markera').addClass('z-depth-3'); $('#markera').addClass('triangle-up'); 
	var style = $('#markera').attr('style'); style += ";background-image:url('res/img/seal3.png');background-size:cover;border:solid 2px darkgrey;border-radius:50%;"; $('#markera').attr('style',style); 
	var markup = '<div style="display:flex;flex-direction:column;align-items:center">' + makeCard('indianred') + '</div><div style="margin-top:16px; font-size:16px;"><span onclick="showDoc(\'tact/a\');" style="cursor:pointer;">Hold Deed</span><br/><br/><a id="butIntro" class="waves-effect waves-light btn red lighten-2" onclick="popBase.togglePopup(); showIntroArt();"><i class="material-icons right">arrow_forward</i>Next</a></div></div>';
...
function showIntroPath() { clearDraw(); startPoint = base; endPoint = learnPath; showPath('goldenrod'); pather = true;  $("#butOwn").addClass("disabled");
	...
	var markup = '<div style="display:flex;flex-direction:column;align-items:center">' + makeCard('cadetblue') + '</div><div style="margin-top:16px; font-size:16px;"><span onclick="showDoc(\'tact/d\');" style="cursor:pointer;">Trade OCUR</span><br/><br/><a id="butPath" class="waves-effect waves-light btn blue lighten-2" onclick="showIntroMap();"><i class="material-icons right">restart_alt</i>Reset</a></div></div>';
