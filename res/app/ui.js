...
//function change(a){ document.getElementById('vid').poster=a; document.getElementById('vid').src=a.replace('res/raw/','res/media/').replace('.jpg','.mp4'); }
		$.getJSON("https://en.wikipedia.org/w/api.php?origin=*&format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&titles=" + domains[i].desc, function (data) { setDialog("<div style='text-align:center;display:flex;justify-content:center;'><p style='display:flex;justify-content:center;align-items:center;'><a style='font-weight:16px;' onclick='openInNewTab(\"" + domains[i].core.external_link + "\");'>" + domains[i].core.name + "</a>&nbsp;&nbsp; · &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + domains[i].core.collection.name + "&nbsp;&nbsp;<img style='display:flex; cursor:pointer;' class='z-depth-1' width='30' height='30' src='res/img/key/" + getCollect(domains[i].core.collection.slug) + "'/><a></a></p></div><br/><div id='emDisp' style='text-align:center;'>" + embedDisplay + "</div></br><p>" + data + "</p><br/><a>Read more...</a><br/><p><img style='max-width:80%;max-height:600px;object-fit:cover;' src='" + domains[i].res + "' /></p>");  });
