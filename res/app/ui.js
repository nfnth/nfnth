...
function showList(area) {  $("#registry").html(''); $("#registry").hide(); 
    switch (area) {
		...
		if (opens[i].core.slug != "ocur" && opens[i].core.slug != "urland") { addOpen(i); } else { if (opens[i].core.slug == "ocur" && boxer.product == true) { addOpen(i); } else if (opens[i].core.slug == "urland" && boxer.special == true) { addOpen(i); } else { opens[i].checked = false; } } } //adjustSort(false);  
...
function openDomain() { var any = false;
	...
	if (domains[i].core.collection.slug == opens[j].core.slug) { if (opens[j].checked == false) { domains[i].hide = true; } else { domains[i].hide = false; any = true; } }
		      ...
		      if (any) { 
			      $("#selLink").removeClass('disabled'); $("#dropdown-domainer").removeClass('disabled'); $("#switchLink").removeClass('disabled'); } else { $("#selLink").addClass('disabled'); $("#dropdown-domainer").addClass('disabled'); $("#switchLink").addClass('disabled'); $("#registry-domain").html(""); $("#registry-domain").append("<a onclick='$(\"#token-tabs\").tabs(\"select\", \"test1\");' class='collection-item'>No domain found.</a>"); }
...
function addOpen(i){ var extra = ""; if(opens[i].checked == true) { extra = "checked='checked'"; } 
	if (showGridOpen) {
    		$("#registry-open").append("<a class='collection-item open' style='padding-right:8px;'><div style='display:flex;justify-content:space-between;align-items:center;'><div><img style='display:flex; cursor:pointer;' onclick='checkOpenDeed(" + i + ");' class='z-depth-1' width='52' height='32' src='" + opens[i].core.banner_image_url + "'/></div>&nbsp;&nbsp;<div style='display:flex;align-items:center;'><div style='display:flex;cursor:pointer;'><div onclick='addOpenDeed(" + i + ");' style='cursor:pointer; width:100px;font-size: 14px;margin-bottom:0px;' class='z-depth-1 chip collected hoverable'><img src='res/img/key/" + getCollect(opens[i].core.slug) + "'>" + opens[i].core.slug + "</div></div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<div><label><input type='checkbox' " + extra + " id='check" + i + "' onclick='setOpen(" + i + ");' /><span>&nbsp;</span></label></div></div></div></a>");  } else {
		$("#registry-open").append("<a class='collection-item open' style='padding-right:8px;'><div style='display:flex;justify-content:space-between;'><div onclick='checkOpenDeed(" + i + ");' style='display:flex; justify-content:space-between;align-items:center;cursor:pointer;'>" + opens[i].core.name + "</div><div style='display:flex;align-items:center;'><div class='chip z-depth-1 hoverable collected' style='font-size:14px;cursor:pointer;margin-bottom:0px;' onclick='addOpenDeed(" + i + ");'>" + opens[i].core.slug + "&nbsp;·&nbsp;<span style='font-weight:bold;font-size:13px;'>" + opens[i].count + "</span></div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<div><label><input type='checkbox' " + extra + " id='check" + i + "' onclick='setOpen(" + i + ");' /><span>&nbsp;</span></label></div></div></div></div></a>");  } }

function checkOpenDeed(i) { document.getElementById("check"+i).click(); }
function addOpenDeed(i) { $("#somedialog2").css("z-index",1004);
    setDialog("<div style='text-align:center;'><p><span style='font-weight:16px'>" + opens[i].core.name + "</span>&nbsp;&nbsp; · &nbsp;&nbsp;<a style='cursor:pointer;' onclick='openInNewTab(\"https://opensea.io/collection/" + opens[i].core.slug + "\");'>" + opens[i].core.slug + "</a></p></div><br/><div style='text-align:center;'><img class='deed-image' style='width:80%;' src='" + opens[i].core.banner_image_url + "' /></div><br/><br/><div style='text-align:center;'><div style='display:flex;align-items:center;justify-content:center;'><img class='z-depth-2' onclick='openInNewTab(\"https://opensea.io/collection/" + opens[i].core.slug + "\");' style='cursor:pointer; margin-right:12px;display:none;' onload='adjustSize(this);' src='res/img/key/" + getCollect(opens[i].core.slug) + "' /><span class='amber lighten-3 badge z-depth-1' style='color:black!important;font-weight:bold;'>" + opens[i].count + "&nbsp;domain</span></div><br/><br/>" + renderMd(opens[i].core.description) + "</div><br/><br/>"); $('.deed-image').fadeOut(0); setTimeout(function() { $('.deed-image').fadeIn(1000); }, 1000);
