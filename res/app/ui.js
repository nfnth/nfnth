...
function showList(area) {  $("#registry").html(''); $("#registry").hide(); 
    switch (area) {
		...
		if (opens[i].core.slug != "ocur" && opens[i].core.slug != "nfnth") { addOpen(i); } else { if (opens[i].core.slug == "nfnth" && nfnth == true) { addOpen(i); } else if (opens[i].core.slug == "urland" && boxer.special == true) { addOpen(i); } else { opens[i].checked = false; } } } //adjustSort(false);  
